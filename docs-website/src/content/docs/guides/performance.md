...
- WGPU geometry and materials are cached by resource handle and version. The retained UI paint stream is independently content-signatured at the backend boundary, so unchanged commands also reuse CPU/GPU vertices without rebuilding or uploading them.
