...
- On adapters with indirect-first-instance, retained batches with at least two instances refine visible objects into resource-owned meshlets. Single-sided camera clusters use sphere, normal-cone, and Hi-Z tests; shadow clusters use cascade spheres. Fixed native multi-draw submits those retained meshlet ranges, while single-instance batches keep one whole-primitive indirect command. Meshlet debug views force the detailed path.
