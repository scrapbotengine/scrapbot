...
The collapsible Performance panel gives a compact frame-health view beside the Systems panel. It publishes every five frames and shows observed FPS from wall-clock presentation intervals and `FRAME` from active CPU work, each over a rolling 50-frame window. Surface acquisition, vsync waiting, and other between-frame idle time affect FPS but not `FRAME`.

The panel also shows the latest valid asynchronous GPU frame duration, effective render scale, scene-plus-runtime entity count, retained GPU draw-batch count, Hi-Z state, frustum rejects, whole-object occlusion rejects, and meshlet occlusion rejects. GPU frame time remains `--` until timestamp-query results are available. A draw batch is Scrapbot's GPU-driven geometry/material grouping unit, not a count of every graphics API draw command across every pass.
...
- On adapters with indirect-first-instance, the same pass refines visible objects into resource-owned meshlets. Single-sided camera clusters use sphere, normal-cone, and Hi-Z tests; shadow clusters use cascade spheres. Fixed native multi-draw submits the retained meshlet command range once per geometry/material/LOD batch.
