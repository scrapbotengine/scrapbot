...
Use `mise benchmark-native-queries` for the high-churn native-query fixture. It copies the ECS showcase to a temporary project, raises the emitter to 500 spawns per second, and reports late frame time, packed entities, chunk count, average chunk occupancy, scalar tail lanes, compiled-plan builds, and retained-plan hits. Optional arguments set frame count and spawn rate, for example `mise benchmark-native-queries -- 4000 1000`.
