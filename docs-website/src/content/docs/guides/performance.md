...
- When `gpu_timestamps_supported` and `gpu_timestamps_valid` are true, cull, shadow, depth, world, Hi-Z, bloom, composite, and UI milliseconds come from WebGPU pass timestamps. `gpu_frame_ms` brackets the complete encoded GPU frame with command-encoder timestamps. Adapters without both timestamp capabilities report timing as unsupported. Four-frame readback rings publish completed samples without synchronously waiting on the GPU. Use external GPU tooling for shader costs, bandwidth, occupancy, and deeper captures.
