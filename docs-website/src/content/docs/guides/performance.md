...
The panel also shows the latest valid asynchronous GPU frame duration, effective render scale, scene-plus-runtime entity count, retained and camera-visible GPU batch counts, visible meshlet draws, Hi-Z state, frustum rejects, whole-object occlusion rejects, and meshlet occlusion rejects. GPU frame time remains `--` until timestamp-query results are available.
