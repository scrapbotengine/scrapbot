...
`scrapbot import`, `check`, `run`, and `build` share the incremental importer and write versioned products under `.scrapbot/imported/`. Source and dependency content, normalized settings, and importer schema participate in the cache key.

Model products use Scrapbot's common validated product envelope. Cached startup reads compiled runtime data and never reparses the source glTF. Ordinary frames never scan, decode, or simplify the asset tree.
