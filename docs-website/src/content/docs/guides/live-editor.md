...
The top bar contains the Scrapbot title and project simulation controls. The bottom bar reports simulation and persistence status. Running and paused playback display `PLAY MODE / <STATE> / ELIGIBLE EDITS PERSIST ON STOP` and add the staged edit count after an eligible edit. The active Play or Pause control turns amber, while a thin viewport frame reinforces that Play Mode is active without obscuring the scene. Stop restores the playback baseline, then reapplies eligible staged edits as one authoring transaction.
...
| `Cmd/Ctrl+.` / `F8` | Stop playback, restore the in-memory authoring baseline, and reapply eligible staged edits as one undoable authoring transaction. |
| `Cmd/Ctrl+D` | Duplicate the complete selection, preserving selected subtrees, and select the explicit copies. The batch is authored while stopped; authored copies are staged during playback while runtime copies remain disposable. |
...
- eligible staged inspector, transform, and authored-duplication edits are reapplied as one undoable authoring transaction;
...
Project resources participate in the same authoring state. Open the Resources dock to browse nested folders under `resources/`. Use `<` to move to the parent directory and `R` to refresh explicit filesystem changes. The breadcrumb reports the current rooted path and direct-item count. Folder and non-renderable resource rows use type icons; laid-out Texture, Model, and Material rows generate revision-cached thumbnails on demand under a fixed GPU-memory budget. Large Model thumbnails reuse their bounded virtual-geometry proxy when available. Rows retain a type-icon fallback while a thumbnail is loading or when the device cannot admit a preview. Interactive Inspector previews use separate live targets, so the number of browser rows does not displace them.

The dock composes the public filter input, virtualized selectable list, and scroll-area components with Create, Duplicate, Delete, and Reimport All controls. Rows show resource type and display name. Type a case-insensitive substring to match either without changing resource selection or registry state. Folder discovery reads metadata only; it does not load extra asset payloads.
...
The Copy, Cut, and Paste icon actions operate on complete authored subtree selections. `Cmd/Ctrl+C` captures selected roots and their descendants; `Cmd/Ctrl+X` removes the same closure; `Cmd/Ctrl+V` creates independent UUIDs, preserves internal hierarchy, and selects the new roots. Paste first validates destination component schemas and project-resource UUIDs, so an incompatible cross-scene clipboard is rejected without partially changing the scene. During Running or Paused playback, Paste, Cut, and Delete of authored entities are staged to keep on Stop; runtime-entity deletion remains disposable. Mint toolbar treatment identifies mutating structural actions that can persist in the current selection context. Focused text and numeric inputs keep their ordinary clipboard behavior instead of invoking scene actions.
...
While stopped, scene entities may use only scene parents, and one completed drag is one undoable, saveable structural transaction. Save emits TOML blocks in authored order without moving live ECS storage handles. During playback, hierarchy and order edits are disposable.

The compact icon toolbar creates a scene entity with a Transform, duplicates the selected scene or runtime entities, removes the selection, or explicitly pins the active runtime entity into scene data. Create and pin are available while stopped. Duplicate and Delete treat selected parents and their Transform descendants as structural subtrees and create one undoable authored transaction while stopped. Duplication preserves hierarchy by remapping parent UUIDs within each copied subtree, then selects the explicitly requested copies. Undo/Redo restores the corresponding ordered selection. While Running or Paused, duplicating or pasting an authored subtree stages creation for Stop, and deleting or cutting one stages removal. Copies and removals of runtime entities remain disposable. `Cmd/Ctrl+D` and unmodified Delete or Backspace use the same toolbar actions. Entity shortcuts are ignored while a text input owns focus. The hierarchy shows scene-authored entities by default, so high-churn runtime spawns do not create thousands of editor rows. A runtime entity selected through the viewport or another tool is surfaced in muted gray and remains fully inspectable. Transient editor-origin entities—including the shell itself and scene camera—stay hidden from the browser and inspector.
...
Browse, Resources, Game, and Inspect are public dock items. Resources shares the left dock space with Browse and opens on demand. Browse contains Performance, Systems, and Scene; Game keeps its viewport fixed as the first stack item while still accepting tool panels. Inspect gives stable order to its identity and runtime-reflected component panels.
...
Drag from empty Game viewport space to draw a clipped selection rectangle. On release, every rendered entity whose complete projected bounds fit inside the rectangle and has at least one actually visible rendered pixel inside it becomes selected; merely touching the rectangle is insufficient, and fully hidden entities are excluded. Generated Model primitives are combined under their root for containment and identity. Hold Shift while dragging to toggle those entities against the current set. Motion must cross four pixels before the gesture becomes a box, so an ordinary click still performs precise rendered-identity picking and never leaks through after a marquee selection. Camera and light icons inside the rectangle participate too.
...
Every component card uses a semantic outline and tinted title to make persistence scannable. Green means changes are authored or `KEPT ON STOP`; amber means temporary runtime state that resets on Stop; muted gray means read-only. During Running or Paused, authored components on authored entities remain editable until a project system actually changes that component on that entity. A system declaring Transform write access for one camera therefore does not lock unrelated rocks, cliffs, or trees. Runtime entities and derived components remain read-only. A short status row remains as a secondary textual explanation. Eligible completed edits are staged per entity/component. Stop restores the playback baseline and reapplies those staged values as one undoable authoring transaction; if a system claims a staged entity/component later in the same play session, that conflicting staged edit is discarded. Duplicating authored subtrees is also staged and kept on Stop. Other entity/component structure and project resources remain playback-only for now.
...
Move the pointer as soon as you press `G`, `R`, or `S`. Each active gesture paints a reference guide: translation connects the starting pivot to its current position, rotation connects the pivot to the captured pointer, and scale connects the pivot to the pointer while marking its starting pointer position. Axis and plane constraints tint the guide with their gizmo color. Scale follows the pointer's distance from the pivot, so moving away enlarges and moving toward it shrinks on every side. Rotation follows the pointer's angle around its pivot. If `R` starts too close to the pivot to define an angle, the first outward motion establishes a neutral lever arm; angular motion after that rotates the selection. Pressing an axis key during the gesture re-evaluates the complete transform from its starting value under that constraint, like Blender's modal transform map. The cursor remains visible, stays confined to the editor window, and wraps to the opposite edge when it reaches a boundary, so a long transform can continue without running out of pointer travel. Click or press Enter to commit; Escape restores the transform from before the chord. An uppercase axis means the Shift-modified excluded-plane form, not a different coordinate system. Axis constraints use the current World or Local orientation.
