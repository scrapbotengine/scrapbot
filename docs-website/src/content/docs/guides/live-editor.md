...
Project resources participate in the same authoring state. Save addresses entities and resources by stable UUID, prepares every dirty scene and resource output in memory, validates the generated TOML and scene resource references, and then commits the complete file set as one recoverable project transaction. A failure before commit restores every previous file; an interrupted committed Save finishes cleanup when the project next loads. Resource names and paths can change without changing scene identity.
