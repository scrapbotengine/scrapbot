...
The normal Odin suite also includes a persistence torture harness. It drives seeded edits through a 512-entity scene, asserts that candidate classification scales with unique dirty UUIDs, keeps value-only comments and formatting intact during mixed structural saves, requires byte-identical repeated saves, excludes runtime entities, round-trips every scene component through structural serialization, injects write and generated-TOML failures, and crosses Save/Undo/Redo/Revert savepoints. These are structural and golden-text assertions rather than machine-dependent timing thresholds.
