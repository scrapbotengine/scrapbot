...
```odin
icon := scrapbot.ui_icon_default()
icon.icon_set = scrapbot.ui_builtin_icon_set()
icon.color = {0.65, 1.1, 1.8, 1}
icon_payload, icon_ok := scrapbot.ui_icon(icon, "play")
if !icon_ok {
	return "native UI icon symbol exceeds the ABI buffer"
}
```
