...
The live editor's Resources dock mirrors nested folders under `resources/` through a rooted, metadata-only browser. It excludes hidden files and symbolic links, lists directories before resources, and recognizes `.resource.toml` files without reading their payloads. Back and explicit Refresh update the current directory; stable frames do no filesystem scan. Rows use atlas-backed folder/type icons or, for laid-out Texture, Model, and Material resources, cached thumbnails from the fixed embedded-viewport pool. Large Models reuse bounded virtual-geometry proxies when possible; the type icon remains the fallback when a preview is not admitted.

The browser creates, duplicates, renames, moves, and deletes material resources as stopped-mode in-memory authoring transactions. Scene references remain stable because these operations preserve the resource UUID. Delete is unavailable while a live entity references the UUID. Explicit Save derives the required file writes and deletions from the disk baseline, rejects destination conflicts, and commits the complete project file set through the recoverable Save transaction. Geometry LOD and UI-theme resources are text-authored in this slice; the editor lists and inspects themes read-only.
...
Adapters without indirect-first-instance and capacity-limited layouts display indexed compatibility
geometry instead of claiming virtual-geometry selection occurred. Streamed resources use their
pinned coarse proxy, while complete resources retain their canonical or imported-LOD fallback.
