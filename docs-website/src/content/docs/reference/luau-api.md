...
UI query payloads expose the same complete layout, value, and style fields used by the editor. `scrapbot.ui_icon` selects an icon-set UUID plus symbol and supports HDR tint/inset; use `scrapbot.ui.builtin_icon_set` for the embedded catalog. Buttons carry the same icon reference plus leading/trailing position, size, gap, and inset.
...
The icon stays fixed while editable text scrolls.
