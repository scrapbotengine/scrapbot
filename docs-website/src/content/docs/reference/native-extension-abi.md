...
Text content, icon symbols, font resource names, and input prefixes use separate bounded byte arrays inside the payload instead of allocator-owned Odin strings. The current limits are 1,023 text bytes, 255 font-name bytes, 63 prefix bytes, and 63 icon-symbol bytes. Callers must copy values they want to retain; the payload itself remains caller-owned.
