...
- Virtual-geometry fields distinguish retained topology, page residency, the active drawable frontier, its projected-error overlap, and refinements in their bounded parent-to-child admission handoff. They expose page budget, resident bytes, total/resident/pinned pages, request/feedback counts, overflow, cumulative page/group uploads, drawable-group activations, `virtual_geometry_transitioning_groups`, `visible_virtual_blend_clusters`, evictions, upload bytes, and deferred group admissions. On the portable compact path, `shadow_visible_meshlets` remains zero while `shadow_visible_instances` reports conservative indexed object-LOD cascade visibility.
