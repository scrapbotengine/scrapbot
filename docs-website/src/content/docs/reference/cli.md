...
JSON run results also include `render_stats`. For WGPU it reports `gpu_driven`, `instance_capacity`, the occupied `instance_slots` span, aligned `visible_capacity`, and cumulative `instance_uploads`/`instance_upload_bytes`. These are CPU-side submission counters; Scrapbot does not stall for visible counts or GPU timings.
