...
Play captures the current authoring state as an in-memory playback baseline. Stop restores that baseline, discards simulation mutations and runtime entities, then keeps eligible staged editor edits. Authored component edits and authored Duplicate, Paste, Delete, and Cut operations can be staged during Running or Paused. Runtime-entity deletion, hierarchy changes, system-written components, and resource changes remain disposable. The Scene pane distinguishes authored entities from runtime entities and hides editor entities.
...
- Visibility capacity: `candidate_record_overflow`, `visible_record_overflow`, and `shadow_record_overflow` count GPU records rejected because their retained slice was full. Healthy frames report zero. `shadow_visible_meshlets` reports ordinary or page-local compact shadow records; fully resident portable virtual resources may instead report conservative indexed object-LOD visibility through `shadow_visible_instances`.
...
Runs a bounded headless WGPU measurement after an excluded warmup. The output bundle contains raw exact-frame CPU/GPU telemetry in `profile.json` and a final `overview.png`. Profile schema version 2 defines GPU frame and scene time as ordered pass-boundary spans; version 1 used an additive pass total and is intentionally not comparison-compatible. A capture range triggers a fresh replay and writes a lossless PNG sequence. The replay bounds its otherwise unpaced GPU queue before capture begins, and all queue and pixel-readback stalls stay outside the measured pass.
