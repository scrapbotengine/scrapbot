...
- Editor state and editability must be understandable at a glance. Prefer semantic icons, color, shape, and control treatment as the primary cue; use concise text only as a secondary explanation or accessibility fallback. Never rely on prose alone for mixed values, read-only state, persistence, validation, selection, or active transport state.
