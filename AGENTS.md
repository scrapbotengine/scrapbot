...
Luau is the target scripting language. The current implementation parses a constrained Luau declaration surface for `ecs.component(...)` and `ecs.system(...)`. The example project declares a project-local `spin` component and executes the first constrained Luau system body shape, `world.rotate("machina.transform", "spin", dt)` with an optional `dt * scalar` multiplier, through the native scheduler. Keep the public API system-first: scripts declare component access, and the native engine owns scheduling, validation, reload transactions, and future parallelization.
