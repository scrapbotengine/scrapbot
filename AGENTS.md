...
- Use `ui_layout.routePointer` as the default public entry point when a feature needs to decide command, scroll, and capture ownership from one pointer frame. Keep lower-level `commandAt` and `routeScrollWheelAt` behavior consistent with it.
...
- Editor chrome pointer handling should build retained `machina.ui.*` routing data and resolve it through `ui_layout.routePointer`; do not add separate editor-only hit-test ladders for new buttons, splitters, scroll views, or future focus/capture controls.
...
- Route scroll-wheel handling through `ui_layout.routePointer`, `ui_layout.routeScrollWheelAt`, or `ui_layout.applyScrollWheelAt` against retained `machina.ui.scroll_view` data. Do not add editor-only or example-only scroll hit tests when a generated UI component can represent the target.
- Route button command handling through `ui_layout.routePointer`/`commandAt` against retained `machina.ui.rect`, `machina.ui.hit_area`, `machina.ui.button`, and `machina.ui.command` data. Editor chrome may consume editor commands directly, but it should still use generated retained UI components for hit ownership.
...
- Prefer reusable editor UI construction helpers that emit ordinary ECS UI entities over hand-coded create/set/find sequences for each panel. The helper is acceptable only if the output is still public retained UI data, not a private widget tree.
