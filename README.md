...
- GPU-driven pipeline: persistent slot-addressed instance storage, dirty-only transform uploads, a growing retained draw database, compute camera/shadow frustum culling, depth prepass with adaptive Hi-Z occlusion, screen-radius object LODs, and indirect draws with asynchronous GPU timing readback. Adapters with indirect-first-instance select a crack-aware virtual-geometry cluster frontier from projected error. Coarse index pages stay pinned; bounded finer pages stream from GPU feedback and fall back without holes. Native multi-draw submits retained per-cluster commands, while portable adapters compact selected camera clusters into shared GPU record streams and vertex-pull compatible spans.
