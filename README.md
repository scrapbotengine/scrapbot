...
- GPU-driven pipeline: persistent slot-addressed instance storage, dirty-only transform uploads, a growing retained draw database, compute camera/shadow frustum culling, depth prepass with adaptive Hi-Z occlusion, screen-radius object LODs, and indexed indirect draws with asynchronous GPU timing readback. Native adapters with indirect-first-instance and native multi-draw select a crack-aware, fully resident virtual-geometry cluster frontier from projected geometric error. Other adapters retain object-level LODs so cluster command expansion cannot regress CPU frame time.
