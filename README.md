...
- GPU-driven pipeline: persistent slot-addressed instance storage, dirty-only transform uploads, a growing retained draw database, compute camera/shadow frustum culling, depth prepass with adaptive Hi-Z occlusion, screen-radius object LODs, and indirect draws with asynchronous GPU timing readback. Adapters with indirect-first-instance select a crack-aware virtual-geometry cluster frontier from projected error. Coarse pages stay pinned; visible demand and bounded future-camera prefetch stream finer vertex/index groups atomically from GPU feedback, with demand-first admission, visible-use hysteresis, and hole-free fallback. Newly drawable refinements replace their complete parent through a bounded complementary screen-door transition shared by world, depth, and shadow passes. Imported Geometry loads a compact runtime catalog directly from its product, retains a position-only CPU query proxy instead of complete render vertices and source indices, and reconstructs exact leaf topology from pages only while establishing a compatibility GPU cache. Native multi-draw submits retained per-cluster commands, while portable adapters compact selected camera clusters into shared GPU record streams and vertex-pull compatible spans.
