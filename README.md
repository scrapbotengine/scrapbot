...
- GPU-driven pipeline: persistent slot-addressed instance storage, dirty-only transform uploads, a growing retained draw database, compute camera/shadow frustum culling, depth prepass with adaptive Hi-Z occlusion, screen-radius object LODs, and indirect draws with asynchronous GPU timing readback. Adapters with indirect-first-instance select a crack-aware, fully resident virtual-geometry cluster frontier from projected geometric error. Native multi-draw submits retained per-cluster commands; portable adapters compact selected clusters into shared GPU record streams and render them through vertex pulling, without CPU command expansion.
