...
Use `tests/fixtures/ui/editor-shortcuts.json` when changing editor visibility, workspace, or transport command shortcuts. It drives the same editor keyboard input used by the platform, verifies Pause and resume, proves that closing a paused shell resumes playback before reopening pauses it again, and crosses both sidebar toggles. Pair it with the state-machine and playback-restoration unit tests for Stop, Step, and world replacement.
