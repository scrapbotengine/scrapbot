...
host_target :: proc() -> string {
	when ODIN_OS == .Darwin {
		when ODIN_ARCH == .arm64 {return "darwin_arm64"}
		else when ODIN_ARCH == .amd64 {return "darwin_amd64"}
	} else when ODIN_OS == .Linux {
		when ODIN_ARCH == .arm64 {return "linux_arm64"}
		else when ODIN_ARCH == .amd64 {return "linux_amd64"}
	} else when ODIN_OS == .Windows {
		when ODIN_ARCH == .arm64 {return "windows_arm64"}
		else when ODIN_ARCH == .amd64 {return "windows_amd64"}
	}
	return "unknown"
}
...
package_project :: proc(root: string, config: Package_Config) -> Package_Result {
	...
	if target != host_target() {
		result.err = clone_package_string(fmt.tprintf(
	...
	if os.exists(output_dir) {
		if err := os.remove_all(output_dir); err != nil {
			result.err = clone_package_string(fmt.tprintf("failed to clean package output: %v", err))
			return result
		}
	}
	...
	if exe_err != nil {
		result.err = clone_package_string(fmt.tprintf("failed to locate Scrapbot executable: %v", exe_err))
		return result
	}
	...
	if err := os.copy_file(executable_path, executable_source); err != nil {
		result.err = clone_package_string(fmt.tprintf("failed to copy packaged executable: %v", err))
		return result
	}
	...
	if err := os.write_entire_file(marker_path, fmt.tprintf("schema_version=1\ntarget=%s\n", target)); err != nil {
...
copy_project_payload :: proc(root, output_dir: string) -> string {
	...
	for entry in entries {
		if entry.name == "build" || entry.name == "native" || entry.name == "types" ||
		   entry.name == ".git" || entry.name == ".vscode" {continue}
...
copy_active_extensions :: proc(src, dst: string) -> string {
	if err := os.make_directory_all(dst); err != nil {return fmt.tprintf("failed to create packaged extension directory: %v", err)}
	...
	if !os.exists(manifest_src) {return "native extension build did not produce an active extension manifest"}
	manifest, read_err := os.read_entire_file(manifest_src, context.temp_allocator)
	if read_err != nil {return fmt.tprintf("failed to read active extension manifest: %v", read_err)}
	...
	if err := os.copy_file(manifest_dst, manifest_src); err != nil {return fmt.tprintf("failed to copy active extension manifest: %v", err)}
	...
	for line in strings.split_lines_iterator(&manifest_text) {
		...
		if file_dst_err != nil {delete(file_src); return "failed to allocate active extension destination path"}
		copy_err := os.copy_file(file_dst, file_src)
		delete(file_src)
		delete(file_dst)
		if copy_err != nil {return fmt.tprintf("failed to copy active native extension: %v", copy_err)}
	}
	return ""
}
...
copy_package_entry :: proc(src, dst: string, entry_type: os.File_Type) -> string {
	...
	case .Directory:
		if err := os.make_directory_all(dst); err != nil {return fmt.tprintf("failed to create package directory: %v", err)}
		entries, read_err := os.read_all_directory_by_path(src, context.allocator)
		if read_err != nil {return fmt.tprintf("failed to read package source directory: %v", read_err)}
		defer os.file_info_slice_delete(entries, context.allocator)
		for entry in entries {
	...
	case .Regular:
		if err := os.copy_file(dst, src); err != nil {return fmt.tprintf("failed to copy package file: %v", err)}
	case:
...
package_executable_name :: proc(name: string) -> string {
	...
	last_was_dash := false
	for rune in name {
		if rune >= 'a' && rune <= 'z' || rune >= '0' && rune <= '9' {strings.write_rune(&builder, rune); last_was_dash = false}
		else if rune >= 'A' && rune <= 'Z' {strings.write_rune(&builder, rune + ('a' - 'A')); last_was_dash = false}
		else if strings.builder_len(builder) > 0 && !last_was_dash {strings.write_rune(&builder, '-'); last_was_dash = true}
	}
