...
Scrapbot embeds Inter 4.1 as the always-available default and fallback for UI text. `inter_regular.ttf` is the source font, `inter_mtsdf.bin` is a 512×512 linear RGBA8 MTSDF atlas, and `../font_data.odin` contains the matching ASCII glyph metrics. Project fonts use the separate automatic `.scrapbot/cache/fonts/` pipeline described by ADR-013.
