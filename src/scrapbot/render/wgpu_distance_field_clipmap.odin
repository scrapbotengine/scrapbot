...
wgpu_distance_field_clipmap_needs_rebuild :: proc "contextless" (
...
wgpu_create_distance_field_clipmap_pipeline :: proc(renderer: ^WGPU_Renderer) -> string {
	...
	if renderer.gpu_distance_field_clipmap_raster_pipeline == nil ||
...
wgpu_encode_distance_field_clipmap :: proc(
	...
	if !wgpu_distance_field_clipmap_needs_rebuild(
	...
	if len(groups) + WGPU_DISTANCE_FIELD_CLIPMAP_JUMP_PASSES + 1 >
	...
	uniforms := make(
		[]WGPU_Distance_Field_Clipmap_Uniform,
		1 + len(groups) + WGPU_DISTANCE_FIELD_CLIPMAP_JUMP_PASSES,
		context.temp_allocator,
	)
	defer delete(uniforms, context.temp_allocator)
	uniforms[0] = wgpu_distance_field_clipmap_metadata(centers, viewport)
	for group, index in groups {
		geometry, _ := resources.get_geometry(registry, group.handle)
		uniforms[1 + index] = uniforms[0]
		uniforms[1 + index].field_dimensions = {
	...
	for step, index in steps {
		uniforms[1 + len(groups) + index] = uniforms[0]
		uniforms[1 + len(groups) + index].control.x = step
	}
	if len(slots) > 0 {
	...
	for group, index in groups {
		...
		dynamic_offset := [?]u32{u32((1 + index) * WGPU_DISTANCE_FIELD_CLIPMAP_UNIFORM_ALIGNMENT)}
	...
	for step_index in 0 ..< WGPU_DISTANCE_FIELD_CLIPMAP_JUMP_PASSES {
		...
		dynamic_offset := [?]u32 {
			u32((1 + len(groups) + step_index) * WGPU_DISTANCE_FIELD_CLIPMAP_UNIFORM_ALIGNMENT),
		}
		...
		wgpu.ComputePassEncoderDispatchWorkgroups(
			pass,
			(WGPU_DISTANCE_FIELD_CLIPMAP_VOXELS + 63) / 64,
			1,
			1,
		)
	...
	renderer.gpu_distance_field_clipmap = {
		...
		centers = centers,
		viewport = viewport,
		rebuild_count = renderer.gpu_distance_field_clipmap.rebuild_count + 1,
...
fn propagate_seeds(@builtin(global_invocation_id) global_id: vec3<u32>) {
	if (global_id.x >= CASCADE_VOXELS * CASCADE_COUNT) { return; }
	let cascade = global_id.x / CASCADE_VOXELS;
	...
	var best = input_seeds[global_id.x];
...
fn finalize_distances(@builtin(global_invocation_id) global_id: vec3<u32>) {
	...
	let cascade = global_id.x / CASCADE_VOXELS;
	let linear = global_id.x % CASCADE_VOXELS;
	let seed = final_seeds[global_id.x];
	if ((seed & 0x80000000u) == 0u) { final_distances[global_id.x] = bitcast<u32>(1e20); return; }
	...
	final_distances[global_id.x] = bitcast<u32>(distance);
}
