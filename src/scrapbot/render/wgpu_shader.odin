...
struct Input {@location(0) position:vec2<f32>,@location(1) uv:vec2<f32>,@location(2) color:vec4<f32>,@location(3) kind:f32,@location(4) size_radius:vec3<f32>,@location(5) clip:vec4<f32>,@location(6) border_color:vec4<f32>,@location(7) border_width:f32,@location(8) border_dash:vec3<f32>,@location(9) font_layer:f32};
struct Output {@builtin(position) position:vec4<f32>,@location(0) uv:vec2<f32>,@location(1) color:vec4<f32>,@location(2) kind:f32,@location(3) size_radius:vec3<f32>,@location(4) @interpolate(flat) clip:vec4<f32>,@location(5) border_color:vec4<f32>,@location(6) border_width:f32,@location(7) border_dash:vec3<f32>,@location(8) @interpolate(flat) font_layer:f32};
@vertex fn ui_vs(input:Input)->Output {var output:Output;output.position=vec4<f32>(input.position,0.0,1.0);output.uv=input.uv;output.color=input.color;output.kind=input.kind;output.size_radius=input.size_radius;output.clip=input.clip;output.border_color=input.border_color;output.border_width=input.border_width;output.border_dash=input.border_dash;output.font_layer=input.font_layer;return output;}
...
fn rectangle_perimeter_coordinate(point:vec2<f32>,size:vec2<f32>)->f32{let edge=vec4<f32>(point.y,size.x-point.x,size.y-point.y,point.x);let nearest=min(min(edge.x,edge.y),min(edge.z,edge.w));if nearest==edge.x{return point.x;}if nearest==edge.y{return size.x+point.y;}if nearest==edge.z{return size.x+size.y+(size.x-point.x);}return size.x+size.y+size.x+(size.y-point.y);}
fn border_dash_coverage(point:vec2<f32>,size:vec2<f32>,dash:vec3<f32>)->f32{if dash.x<=0.0||dash.y<=0.0{return 1.0;}let period=dash.x+dash.y;let coordinate=rectangle_perimeter_coordinate(point,size);let phase=fract((coordinate+dash.z)/period)*period;let center_distance=abs(phase-dash.x*0.5);let wrapped_distance=min(center_distance,period-center_distance);let signed_distance=wrapped_distance-dash.x*0.5;let smoothing=max(fwidth(coordinate),0.75);return 1.0-smoothstep(-smoothing,smoothing,signed_distance);}
