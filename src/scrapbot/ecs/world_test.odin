...
test_scene_builds_world_with_soa_transforms :: proc(t: ^testing.T) {
	...
	autorotate := find_custom_component_storage(&world, shared.INVALID_COMPONENT_ID, "autorotate")
	clock := find_custom_component_storage(&world, shared.INVALID_COMPONENT_ID, "scrapbot.clock")
	testing.expect(t, autorotate != nil && clock != nil)
	testing.expect(t, len(autorotate.components) == 1)
	testing.expect(t, autorotate.components[0].entity_index == 1)
	testing.expect(t, len(clock.components) == 1)
	testing.expect(t, clock.components[0].entity_index == 0)
