...
reconcile_editor_scene_camera :: proc(world: ^World, enabled: bool) -> (int, ^shared.Editor_Scene_Camera_Component, bool) {
	if world == nil {
		return -1, nil, false
	}
	...
	camera := shared.Camera_Component{fov = 60, near = 0.1, far = 100}
...
editor_scene_camera_entity :: proc(world: ^World) -> (int, ^shared.Editor_Scene_Camera_Component, bool) {
	if world == nil {
		return -1, nil, false
	}
	for &component in world.editor_scene_cameras {
		index := component.entity_index
		if index >= 0 && index < len(world.entities) && world.entities[index].alive && world.entities[index].origin == .Editor {
...
editor_scene_camera_system :: proc(world: ^World, input: shared.Editor_Fly_Camera_Input, delta_seconds: f32, enabled: bool) {
