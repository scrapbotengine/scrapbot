	...
	switch term.name {
		...
		case "scrapbot.camera":
			if entity.camera_index >= 0 && entity.camera_index < len(world.cameras) {
				...
				debug_view := shared.render_debug_view_name(value.debug_view)
				lua_pushlstring(L, cstring(raw_data(debug_view)), c.size_t(len(debug_view)))
				lua_setfield(L, -2, "debug_view")
				lua_pushnumber(L, f64(shared.camera_debug_hiz_mip(value)))
				lua_setfield(L, -2, "debug_hiz_mip")
				lua_pushboolean(L, 1 if value.debug_occlusion_freeze else 0)
				lua_setfield(L, -2, "debug_occlusion_freeze")
				lua_pushnumber(L, f64(shared.camera_resolution_scale(value)))
				lua_setfield(L, -2, "resolution_scale")
				lua_pushboolean(L, 1 if value.dynamic_resolution else 0)
				lua_setfield(L, -2, "dynamic_resolution")
				lua_pushnumber(L, f64(shared.camera_dynamic_resolution_min_scale(value)))
				...
				lua_setfield(L, -2, "screen_space_reflections_quality")
...
push_ui_dock_space_table :: proc "c" (L: Lua_State, value: shared.UI_Dock_Space_Component) {
	...
	push_number_field(L, "tab_connection_height", value.tab_connection_height)
	push_number_field(L, "tab_content_overlap", value.tab_content_overlap)
	push_vec4_field(L, "tab_strip_background", value.tab_strip_background)
	push_vec4_field(L, "content_background", value.content_background)
	push_number_field(L, "content_corner_radius", value.content_corner_radius)
	push_vec4_field(L, "content_padding", value.content_padding)
