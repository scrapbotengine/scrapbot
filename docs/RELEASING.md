...
3. Review the release PR. It should update `CHANGELOG.md`, `version.txt`, `src/root.zig`, `build.zig.zon`, and `.release-please-manifest.json`.
...
The release workflow uses `GITHUB_TOKEN`. Workflows triggered by release-please-created tags or releases should not be expected to run from separate `release` events; release artifacts are built in the same workflow after release-please reports that it created a release.
