...
- The Game viewport has a public-ECS popup selector for `Camera`, Lit, Base Color, World Normals, Roughness, Metallic, Depth, Meshlets, LOD, and Meshlet Visibility. `Camera` follows the active camera's authored `debug_view`; another choice overrides only the editor's extracted camera copy, so inspection never enters history or marks the scene dirty. The same camera contract remains available to projects without the editor.
...
- Render initial FPS, active CPU, GPU, project-entity, draw-batch, frustum-rejection, and occlusion-rejection values through an editor-origin collapsible panel composed only from public panel, table, VStack, layout, and text components.
