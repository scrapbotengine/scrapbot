...
- The inspector shows the selected entity's editable name, stable UUID, provenance, attached components, field names, and current values. Cards appear entirely from live registry membership plus runtime inspection of each canonical typed payload; project/native dynamic components use their registry schemas as runtime type metadata. No component owns an editor panel builder or hand-authored row list. Components are vertically stacked collapsible titled panels, and each panel renders its discovered fields in an edge-to-edge, resizable two-column property table with a one-third label column and two-thirds value column inside an independently scrollable sidebar. Individual label and value cells provide the horizontal inset, keeping the table geometry aligned with the panel while its contents retain comfortable spacing. Registry definitions may mark bookkeeping-heavy components as advanced; those components remain visible and fully inspectable but start collapsed whenever a pooled panel is rebound to them, while a user's expansion persists for the current binding. Bool, String, Number, Vec2, Vec3, Vec4, and Color values select reusable checkbox and input controls by type; vectors compose two to four equal-width axis inputs with a fill HStack. Marker components naturally produce title-only cards. UUID-backed strings and text-alignment values use validated text until specialized entity and enum pickers exist. Field metadata provides validation, staged keyboard editing, and opt-in live scrubbing. Engine-managed derived values and unsupported payload types remain visible and read-only. An Add Component button opens a floating, independently scrollable menu composed from the same public layout, scroll-area, VStack, text, and button components. The menu reads the live component registry, reuses cached registry name tokens, rebuilds its sorted definition cache only when the registry revision changes, nests dotted engine and library names by token, groups single-token project components under Project, and lists only components absent from the selected entity. Every removable component panel composes an ordinary direct-child icon button into its title band; activating that reusable button removes the component without toggling collapse. Stopped scene-entity membership changes are exact per-component undoable authoring transactions; running and paused changes apply immediately to scene or runtime entities as disposable playback state. Engine-defined scene components remain mutable because the entity owns their membership; engine-managed derived components remain inspector-visible and have no removal action.
...
**Tradeoff:** Storage-kind adapters still locate canonical payloads, and component validation may specialize by semantic invariant. UUID and alignment values are validated text controls until specialized pickers exist; Color has semantic RGBA storage and bounded channel inputs but not yet a public color-picker control. Arrays, nested values, and unknown types remain visible only when a generic read-only representation exists.
