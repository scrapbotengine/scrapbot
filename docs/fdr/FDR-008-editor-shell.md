...
- The inspector shows the selected entity's editable name, stable UUID, provenance, attached components, field names, and current values. Components are vertically stacked collapsible titled panels, and each panel renders its fields in an edge-to-edge, resizable two-column property table with a one-third label column and two-thirds value column inside an independently scrollable sidebar. Individual label and value cells provide the horizontal inset, keeping the table geometry aligned with the panel while its contents retain comfortable spacing. Registry fields with truthful Bool, String, Number, Vec2, Vec3, or Vec4 schemas are generically editable through reusable checkbox and input controls; vectors compose two to four equal-width axis inputs with a fill HStack. UUID-backed strings and text-alignment values use validated text until specialized entity and enum pickers exist. Existing transform, camera, light, and custom Vec3 adapters retain immediate numeric preview and constraints. Engine-managed derived values and opaque render handles remain selectable and read-only. An Add Component button opens a floating, independently scrollable menu composed from the same public layout, scroll-area, VStack, text, and button components. The menu reads the live component registry, nests dotted engine and library names by token, groups single-token project components under Project, and lists only components absent from the selected entity. Every removable component panel enables the reusable panel title action, rendered as a trailing cross; activating it removes that component without toggling collapse. Stopped scene-entity membership changes are undoable authoring transactions; running and paused changes apply immediately to scene or runtime entities as disposable playback state. Engine-defined scene components remain mutable because the entity owns their membership; engine-managed derived components remain inspector-visible and have no removal action.
...
**Tradeoff:** The registry's namespace and owner identify who defines a component, while the editor maintains a narrower mutation-support policy until serializability and runtime ownership become explicit registry metadata. Stop discards live membership changes together with other playback mutations.
...
- **ADRs:** ADR-003, ADR-005, ADR-014, ADR-016, ADR-017, ADR-018, ADR-019, ADR-021, ADR-023, ADR-024, ADR-025, ADR-026, ADR-027, ADR-028, ADR-031
