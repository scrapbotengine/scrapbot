...
- Selecting a renderable entity or Model root adds an editor-only amber outline around its complete projected silhouette, including portions behind other scene geometry. Selection changes patch only the old and new retained GPU instance records; project materials and authored components remain unchanged.
...
**Decision:** Treat every non-stopped transport state as play mode, including Pause and Step. Keep both application bars on the shared neutral chrome recipe, then compose a persistent amber warning from the shared warning-text and warning-frame recipes: state that changes are temporary, animate a dashed viewport frame while Running, and hold a solid viewport frame while Paused. Stop removes that semantic emphasis when it returns to authoring mode. Implement the motion through the public `ui_layout` dash phase so each running frame invalidates only viewport paint; paused and stopped frames perform no animation work.
