...
- Model resource rows are generic public `ui_action` drag sources. Dropping one on the Game viewport resolves the pointer against scene geometry, while dropping on empty Scene space uses the viewport center. Dropping on a Scene row preserves that world pose and parents the new model to the target entity. Non-Model resource rows do not advertise placement drags.
...
**Decision:** Let the selected Model preview expose an ordinary public button that requests a scene-origin Transform and Model root at the viewport center. Let generic resource drags carry the exact pointer. Resolve both through the renderer's backend-neutral nearest-triangle query, then fall back to the world ground plane or a five-unit camera focus point. Quantize the resolved world position with the editor's public-button grid control when snapping is enabled. Use the asset name, stable resource UUID, project geometry policy, selection, dirty tracking, and the existing structural history path. Offer placement only while stopped.
...
**Tradeoff:** Placement uses exact CPU triangle queries and therefore inherits the current picking path's linear cost. A scalable broad phase or GPU identity query remains necessary for very large scenes. Placement snaps the model origin; imported bounds and orientation do not yet align an asset's base or up axis to the hit surface.
