...
**Decision:** Route command-modified E, R, T, and B through the editor input path as shell, transport, and workspace commands. Opening the shell pauses active playback; closing starts or resumes it. Plain command-B toggles Browse, while Alt distinguishes the Inspect toggle. Ignore command-modified E and R in the unmodified transform-gizmo shortcut path, and gate transport/workspace commands while project text input or fly-camera capture owns the keyboard.
**Why:** Entering the editor should stabilize the inspected world immediately, and high-frequency workspace and playback controls need platform-native shortcuts without stealing ordinary project input or accidentally changing the active transform tool.
...
**Decision:** Carry editor selection into the retained GPU instance record as one transient flag and retain a dense list of selected instance slots. Mark a directly selected renderable and every generated primitive whose `model_owner` is the selected Model root. Draw their complete fallback Geometry into an editor-only mask without the scene depth attachment, then dilate the complete projected mask into a high-contrast amber silhouette in the final composite. Use the mask's second channel for the active placement ghost. Patch records and dense membership only when selection or an already-dirty selected instance changes.
...
**Tradeoff:** The fixed-width screen-space outline intentionally reveals the selected silhouette through occluders. It uses each Geometry's complete conventional fallback rather than its currently selected streamed detail, and bounds feedback draws to 4,096 primitives.
