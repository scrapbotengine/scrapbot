...
- The top bar contains only the Scrapbot brand and Play, Pause, Stop, Step, Undo, Redo, Save, and Revert controls. Play captures the current authoring world and then runs project systems with normal frame deltas; Pause freezes project systems and world time while editor rendering and tools remain responsive; Stop restores the captured in-memory authoring world while retaining loaded Luau and Odin systems, then remains stopped; Step captures authoring state when necessary, advances one fixed 1/60-second project update, and remains paused. Undo and Redo traverse stopped-mode authoring transactions. Save explicitly persists dirty authored scene and resource values, while Revert reloads resources and scene entities from disk without reloading project code.
