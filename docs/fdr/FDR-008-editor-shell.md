...
- Selecting a renderable entity or Model root adds an editor-only amber outline around its complete projected silhouette, including portions behind other scene geometry. A viewport ray hit on a generated Model node or primitive promotes selection to its authored Model root, so the scene browser and inspector never expose derived render entities as the user's selection. Throughout Running and Paused play mode, the outline becomes animated dashes sampled from run-global engine time to communicate that live edits are temporary even when project clocks are frozen. Stopped selection uses the steady solid outline. Hiding the editor suppresses the outline while retaining the selected entity for the next editor session. Selection changes patch only the old and new retained GPU instance records; project materials and authored components remain unchanged.
...
- `Cmd/Ctrl+Z` undoes, `Cmd/Ctrl+Shift+Z` redoes, and `Cmd/Ctrl+S` saves while the editor is open. `Cmd/Ctrl+D` duplicates the selected entity and selects the copy; unmodified Delete or Backspace deletes the selected entity, and Escape deselects it. Focused fields and open popups consume Escape first so canceling an edit or dismissing a menu does not also change selection. Duplicate and Delete are authored and undoable while stopped, and disposable while Running or Paused. `Cmd/Ctrl+R` plays when stopped, resumes when paused, and stops when running. `Cmd/Ctrl+T` pauses a running project and advances one fixed step while paused or stopped. `Cmd/Ctrl+B` toggles Browse, and `Cmd/Ctrl+Alt+B` toggles Inspect.
...
- Hovering emphasizes the nearest axis, plane, or center handle. Dragging captures the pointer and updates position, rotation, or scale according to the gizmo's ECS-visible mode and space. Modal transform chords use the same frozen axes and history path. Transform keys are ignored during RMB fly-camera capture or text focus, and Escape cancellation is consumed before deselection. The orientation toolbar receives pointer input ahead of coincident gizmo handles.
...
- Headless WGPU runs can combine `--editor` with semantic UI action replay, exact fly-camera poses, structured UI-tree dumps, assertions, and target-cropped framegrabs for deterministic editor workflows without OS pointer automation.
