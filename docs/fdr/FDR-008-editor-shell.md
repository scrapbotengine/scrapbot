...
- Selecting a renderable entity or Model root adds an editor-only amber outline around its complete projected silhouette, including portions behind other scene geometry. Throughout Running and Paused play mode, the outline becomes animated dashes driven by renderer frame time to communicate that live edits are temporary even when simulation is frozen. Stopped selection uses the steady solid outline. Hiding the editor suppresses the outline while retaining the selected entity for the next editor session. Selection changes patch only the old and new retained GPU instance records; project materials and authored components remain unchanged.
