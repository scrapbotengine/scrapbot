...
- Selecting a renderable entity or Model root adds an editor-only amber outline around its complete projected silhouette, including portions behind other scene geometry. While Play Mode is Running, the outline becomes animated dashes driven by ECS simulation time to communicate that live edits are temporary. Paused and Stopped selection use the steady solid outline. Selection changes patch only the old and new retained GPU instance records; project materials and authored components remain unchanged.
