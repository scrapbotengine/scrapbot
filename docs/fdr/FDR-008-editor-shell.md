...
- Model resource rows are generic public `ui_action` drag sources. Dropping one on the Game viewport creates a root through that same placement transaction; dropping on empty Scene space creates a root, while dropping on a Scene row preserves the camera-relative world pose and parents the new model to that entity. Non-Model resource rows do not advertise placement drags.
