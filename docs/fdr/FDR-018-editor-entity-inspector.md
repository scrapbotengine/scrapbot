...
- A selected renderable gets a translate gizmo with visible X, Y, and Z handles in Odin software and WebGPU editor render paths. Handles use world axes by default, can be switched to local axes through a top-bar editor chrome control, and can temporarily use local axes while Alt is held when the selected entity has a transform rotation.
- Dragging a gizmo axis mutates the selected entity's transform position. Holding Shift during an Odin translate-gizmo drag snaps the drag distance to 0.25 world-unit increments along the active axis. The current world/local mode is captured at drag start; Alt can temporarily force local axes before the drag. In the Odin implementation, each completed translate-gizmo drag contributes one grouped undo/redo command and one pending scene edit for `scrapbot.transform.position`.
...
**Tradeoff:** Gizmo rendering is currently simple screen-space strokes projected from world-space or local-space axes. Rotation/scale modes, richer snapping controls, occlusion behavior, and a broader editor transaction model are not covered yet.
