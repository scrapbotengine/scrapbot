...
- A selected renderable gets a translate gizmo with visible X, Y, and Z handles in Odin software and WebGPU editor render paths. Handles use world axes by default and Alt-held local axes when the selected entity has a transform rotation.
- Dragging a gizmo axis mutates the selected entity's transform position. Holding Shift during an Odin translate-gizmo drag snaps the drag distance to 0.25 world-unit increments along the active axis. Holding Alt before the drag captures local-space axes for the drag. In the Odin implementation, each completed translate-gizmo drag contributes one grouped undo/redo command and one pending scene edit for `scrapbot.transform.position`.
...
**Tradeoff:** Gizmo rendering is currently simple screen-space strokes projected from world-space or Alt-held local-space axes. Persistent world/local-space mode controls, rotation/scale modes, richer snapping controls, occlusion behavior, and a broader editor transaction model are not covered yet.
