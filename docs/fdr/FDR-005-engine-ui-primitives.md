...
- Headful runs show an engine-owned debug overlay by default; the first overlay displays current FPS over the rendered scene.
...
**Decision:** The first editor/debug overlay is generated by the engine at runtime instead of being authored in the project scene.
