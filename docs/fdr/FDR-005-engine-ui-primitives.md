...
Engine UI primitives provide the controls and layout capabilities needed for runtime overlays, debug tools, and the future editor. They exist so Scrapbot can build tooling with its own scene model, renderer, and input systems instead of depending on a separate editor application stack.
...
- `scrapbot.ui.border` adds a data-authored border to a rect with `color` and `thickness`. Borders render through the same rounded-rectangle SDF path by drawing an outer rounded rect and an inset fill rect.
...
- UI interaction consumes transient `scrapbot.input.*` ECS resources instead of reading raw platform events directly.
...
- Headful runs hide the engine-owned editor/debug overlay by default; `scrapbot run --editor` starts with it visible.
- The engine-owned editor/debug shell displays current FPS in a top bar and keeps its chrome spacing on a 4px grid.
- Editor chrome constants are renderer-space pixels in headful windows so existing Retina visual density is preserved. Offscreen rendering can set an explicit `scrapbot.input.frame.pixel_scale` for DPI verification; final UI vertices are scaled into physical render-target pixels after retained layout resolution only when that scale is greater than `1.0`.
...
- `scrapbot.ui.scroll_view` defines a screen-space viewport with `position`, `size`, and `content_offset` fields. Descendants are offset by `content_offset` and clipped to the viewport.
...
- `scrapbot.ui.layout.item` can also parent a child to a non-container UI rect, text, or separator. In that case the child inherits the parent's resolved position and continues through the parent's layout chain. This is the preferred pattern for button labels and small composite controls.
- `scrapbot.ui.spacer` participates in layout without rendering.
- `scrapbot.ui.text_block` gives a text entity a content box and horizontal/vertical `start`, `center`, or `end` alignment.
- `scrapbot.ui.hit_area` defines a non-rendering interaction rectangle with `position` and `size`. Command routing and button interaction state prefer it over the visual rect when present.
- `scrapbot.ui.toggle` stores checked state and influences button/rect visuals. It does not yet toggle itself automatically; scripts or editor systems own state mutation.
- `scrapbot.ui.progress_bar` stores value, max, and fill color. It renders as a fill inside the entity's rect.
- `scrapbot.ui.separator` renders a thin semantic divider through the same UI vertex path as rectangles.
...
- Mouse-wheel scroll routing is component-based: code asks the shared retained UI router for the `scrapbot.ui.scroll_view` under the pointer, receives the bounded next offset, and then applies or mirrors that result. Scene-authored scroll views apply it directly; editor lists generate small scroll-view routing worlds and mirror the routed offset into animated editor state.
...
**Decision:** Editor and runtime tools are built with Scrapbot UI primitives.
...
**Decision:** The first editor/debug shell is generated by the engine at runtime instead of being authored in the project scene, and it is opt-in at launch through `scrapbot run --editor`.
**Why:** Editor chrome should be available over any project without mutating project files, while still using the same ECS UI primitives and renderer path as scene-authored UI. The game viewport must remain an explicit region so editor panels do not obscure the running scene. It follows ADR-007 and ADR-022.
...
**Why:** Retained ECS data keeps the first slice text-first, reloadable through scene files, render-testable, and aligned with ADR-008 and ADR-022.
...
**Decision:** `scrapbot.ui.rect.corner_radius` is a real ECS field and rounded corners are rendered in `ui.wgsl` through a rounded-rectangle SDF.
...
**Decision:** Button entities can author a command id, and a successful release emits a one-frame command event containing the command id and source entity id. A button can optionally provide `scrapbot.ui.hit_area` when its interaction target should differ from its visual rectangle.
...
**Tradeoff:** The current event shape is intentionally small and does not yet model bubbling, capture, disabled state, modifiers, typed payloads, or persistent focus. Engine-owned editor commands may consume routed hits directly instead of emitting project-world `scrapbot.ui.command_event` data.
...
**Decision:** Scrapbot exposes `scrapbot.ui.scroll_view`, `scrapbot.ui.vgroup`, `scrapbot.ui.hgroup`, and `scrapbot.ui.layout.item` as retained ECS components. The editor performance table and editor shell body use the same public primitives that project scenes can author.
...
### 9a. Add a Scrapbot-native control library shape

**Decision:** Scrapbot expands the retained UI model with `scrapbot.ui.stack`, `scrapbot.ui.spacer`, `scrapbot.ui.text_block`, `scrapbot.ui.toggle`, `scrapbot.ui.progress_bar`, and `scrapbot.ui.separator` instead of adopting Godot's exact control names.
**Why:** Godot's UI model is useful inspiration: content controls, layout containers, child sizing metadata, and themeable semantic controls. Scrapbot still needs component names and behavior that fit ECS authoring, text scenes, and future agent workflows.
...
**Decision:** Scrapbot adds opt-in canvas fit/fill scaling, layout item margins, and rounded rect borders as ECS component fields instead of treating the UI gallery as a window-size-specific hand layout.
...
**Decision:** A `scrapbot.ui.layout.item` child may target a non-container UI rect, text, or separator parent and inherit that parent's resolved position before continuing up the parent's own layout chain.
...
**Decision:** Scrapbot replaces `scrapbot.ui.vbox` with `scrapbot.ui.vgroup`. Vgroups use the same `position`, `size`, `spacing`, `padding`, and `scrapbot.ui.layout.item` grow/shrink negotiation as hgroups, but distribute space along the vertical axis. Simple vertical stacks use `scrapbot.ui.stack`.
...
**Tradeoff:** This is a breaking scene schema change. Old scenes authoring `scrapbot.ui.vbox` must migrate to `scrapbot.ui.vgroup` with explicit `size` and `padding`, or to `scrapbot.ui.stack` when they only need natural directional stacking.

### 10. Treat examples as the primitive gallery

**Decision:** The UI gallery example is the proving ground for retained UI primitives until Scrapbot has a richer widget/layout library.
...
- **ADRs:** ADR-001, ADR-004, ADR-007, ADR-008, ADR-020, ADR-022
...
- What exact ECS shape should grid/flex sizing, scroll bars, asymmetric margins/padding, and grow-ratio space distribution beyond `scrapbot.ui.hgroup` and `scrapbot.ui.vgroup` use?
