...
**Migration note:** During the Odin rewrite, bounded `scrapbot run --frames` can poll default scene, Luau script, and Odin native source changes between frames, revalidate replacements, reload compatible scenes as fresh scene generations, preserve the current world on successful script/native reloads, and keep the last-known-good state active on failed reloads. Unbounded/window-loop integration remains pending.
