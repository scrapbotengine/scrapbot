...
**Tradeoff:** The implementation has fixed node/paint limits. Stable project and editor domains independently signature their resolved nodes, visual components, interaction state, fonts, and split handles; unchanged domains retain their paint-command ranges and skip both traversal and glyph emission. WGPU separately signatures the resulting command stream and viewport so unchanged frames neither rebuild nor upload UI geometry. Every visual, structural, or layout-affecting mutation must reach the retained inputs so both cache layers remain correct. The same reconciler maintains distinct project and transient editor UI coordinate and interaction domains. See ADR-024.
