...
- Transform expansion, clustered-light construction, visibility, shadows, depth, world shading, Hi-Z, AO, SSR, volumetric fog, temporal resolve, bloom, automatic exposure, composite, and UI have distinct timestamp phases. GPU frame time brackets the complete encoded frame. The timing path is enabled only when the adapter exposes both timestamp queries and native command-encoder timestamp writes; otherwise the renderer reports timing as unsupported.
