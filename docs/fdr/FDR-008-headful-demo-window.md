...
**Odin migration note:** The Odin `run` command can execute bounded hidden frame loops and, with `--backend wgpu`, write a final offscreen WebGPU frame artifact through the Odin `wgpu-native` path and present the final simulated scene through a hidden SDL WebGPU surface. Bounded visible software `run --frames` now creates an SDL window, pumps quit/close events, and drives the shared live-project frame tick while pixel presentation remains pending. The Odin SDL boundary can initialize video, create high-DPI windows, extract platform-native `wgpu-native` surface descriptors for Metal, Wayland/X11, or Win32, and present a single hidden clear frame through `scrapbot wgpu-surface-check`. Odin still rejects visible WebGPU presentation, the unbounded WebGPU window loop, and WebGPU editor chrome until the full surface presentation loop is ported.
