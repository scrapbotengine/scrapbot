...
- Publish generic activation, change, submission, cancellation, and drop events from the shared interaction pass into the World-owned immutable event history defined by ADR-042. Project systems, native extensions, and editor orchestration consume that same ordered contract with independent cursors. Do not branch from reusable control mechanics directly into editor commands.
...
- Model browser search and large row sets through public `ui_list` fields: a list references a same-origin public `ui_input` by UUID, filters descendant control content, retains matching tree ancestry, and may opt into uniform-row virtualization with bounded overscan. The shared UI owns dependency invalidation, compact filtered order, and visible-window layout; editor code only composes the input and list.
- Compose floating choices and menus from a root public `ui_layout` marked as a popup, an anchor UUID, ordinary list/scroll content, and a public `ui_button` popup target. Shared UI owns open state, anchor placement, viewport flipping/clamping, outside/Escape dismissal, and opt-in close-on-selection; editor code only binds selected UUIDs to project meaning.
