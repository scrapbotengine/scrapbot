...
The runtime owns one active scene and one project-wide resource registry. Scene discovery metadata is compact: it contains identity, location, and the dependency-indexed resource closure, but no entity payloads. During a transition, only the active world and one candidate world may coexist. ADR-061 defines active/staging resource ownership, shared-resource reuse, and delayed payload eviction.

## Consequences

Scene paths and display names can change without breaking startup configuration or runtime requests. Failed transitions are non-destructive, repeated transitions do not accumulate ECS worlds, and resource-heavy projects do not multiply registry ownership per scene. Candidate construction temporarily raises ECS memory to the sum of the current and next worlds, and the current implementation performs that work synchronously at the frame boundary. Time-budgeted/background preparation, additive worlds, and persistent cross-scene layers remain separate future capabilities.
