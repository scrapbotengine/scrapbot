...
The portable fallback still submits one CPU-known draw per geometry/material/LOD batch. Native adapters with indirect-first-instance support consume resource-owned meshlets and issue one fixed multi-draw over each batch's CPU-known cluster command range; compute writes each command's instance count. It retains an explicit backend limit of 131,072 instance slots and a 1,048,576-entry bound for meshlet visibility before falling back. Cluster-table capacity grows geometrically only while the complete binding remains legal; the final growth step uses the largest capacity permitted by the device's storage-buffer binding limit instead of rejecting an otherwise representable topology. New batch keys, geometry-LOD topology changes, and draw-database growth rebuild batch visibility slices; ordinary membership churn within retained capacity does not. Hi-Z currently requires a stable camera for one frame and project-authored geometry LODs currently generate icosphere levels. Imported mesh LODs, offline simplification, bindless materials, skinning, and a genuinely GPU-authored command topology remain future work.
