...
Evaluate AO at a camera-authored resolution scale, defaulting to quarter resolution, over rotated view-space slices. Replace GTAO's two maximum horizons with a 32-sector visibility bitmask. Every sampled front surface plus a constant-thickness reconstructed back surface marks only its angular interval, allowing visibility to reopen behind thin geometry.
...
Filter and reconstruct AO only through depth- and normal-similar neighbors. Derive representative full-resolution samples from the actual source and destination dimensions rather than a fixed scale. Apply visibility only to indirect diffuse; direct lights, specular lighting, emission, and SSR remain unoccluded.
...
Make world render scale, optional dynamic-resolution policy, automatic exposure, temporal antialiasing, current-frame fast antialiasing, ambient occlusion, screen-space reflections, and bloom authored fields of `scrapbot.camera`. Manual render scale is bounded to `0.5`–`1`, defaults to native resolution, and acts as the dynamic ceiling. Dynamic resolution adds a bounded floor and positive GPU-millisecond target.

Use asynchronously completed WGPU timestamps without waiting on the CPU. Consume every sample individually and exactly once, exclude native UI time, filter the scalable duration, and move in 5% steps with asymmetric hysteresis. Tag submitted samples with a controller generation so delayed results from an old scale, policy, or project camera are rejected. The stable project-camera UUID owns controller identity while the editor fly camera borrows its policy. The effective scale remains backend-derived and never writes into the authored camera. Backends without timestamps use the authored ceiling.

Give AO and SSR four bounded sample-quality tiers from `0.25` to `1`, with balanced `0.5` as the default. SSR maps those tiers to 16, 32, 48, and 64 steps and widens lower-tier strides to preserve approximate ray reach. The active project camera owns the view policy. While the editor fly camera supplies an editor viewport's pose and lens, it inherits the active project camera's complete render policy so live inspector edits remain visible. TAA takes precedence over fast antialiasing when both switches are on. A disabled feature must avoid its unnecessary jitter, history sampling, or compute dispatch rather than merely hiding its output.

Run world shading, depth, Hi-Z, and postprocessing on the scaled grid. Composite that complete grid into the native output before rendering project UI, gizmos, and editor chrome at native resolution. A scale change rebuilds only size-dependent targets and rejects temporal history; an unchanged scale performs no target work.
...
UI remains crisp because it renders at native output resolution after scaled world postprocessing. Headless framegrabs exercise the same deterministic jitter, upscale, and composite path.

The backend owns render-scale-sized surface, indirect-diffuse, and reflection textures; two ping-pong temporal color/depth pairs; several size-dependent floating-point bloom textures; independently resolution-scaled AO and volumetric-fog textures; an optional scaled depth target; dynamic-resolution timing/filter state; automatic-exposure settings/state buffers; compute bind groups; and a final native-output composite pass. Native scale borrows the output depth target.

Window resize, effective-scale change, or replacement of the sampled depth target rebuilds the affected bindings and rejects stale history. Camera fields provide manual/dynamic scale policy, coarse feature switches, AO and SSR sample quality, and fixed or automatic exposure. AO radius/intensity/thickness, SSR distance/thickness/roughness, bloom threshold/intensity/scatter, and temporal history/quality remain future controls. Consolidating compute work avoids the command-finalization cost of many short render passes.
