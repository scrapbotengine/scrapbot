...
The backend owns render-scale-sized surface, indirect-diffuse, and reflection textures; two ping-pong temporal color/depth pairs; several size-dependent floating-point bloom textures; half-resolution AO textures; an optional scaled depth target; automatic-exposure settings/state buffers; compute bind groups; and a final native-output composite pass. Native scale borrows the output depth target. Window resize, render-scale change, or replacement of the sampled depth target rebuilds the affected bindings and rejects stale history. Camera fields now provide render scale, coarse feature switches, AO and SSR sample quality, and fixed or automatic exposure; dynamic resolution, AO radius/intensity/thickness, SSR distance/thickness/roughness, bloom threshold/intensity/scatter, and temporal history/quality remain future controls. Consolidating compute work avoids the command-finalization cost of many short render passes.
