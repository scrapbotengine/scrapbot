...
Machina scripts register ECS component schemas as runtime data, but authors also need useful editor types for component payloads inside systems. Maintaining separate payload type aliases and field maps is repetitive and easy to drift, especially for agent-authored gameplay code.
...
Luau's new type solver supports user-defined type functions that can inspect table-property types and synthesize new table types during analysis. This gives Machina a way to derive editor payload types from literal field schema tables while keeping runtime component declarations text-like and explicit.

## Decision

Machina will use Luau type functions in `types/machina.d.luau` to derive component payload editor types from field declarations created with `ecs.fields(...)`.
...
Machina now depends on Luau new-solver behavior for the richest editor experience. The runtime scripting boundary still works without editor type checking, but contributors need a language server and type-check command that enable `LuauSolverV2`.
