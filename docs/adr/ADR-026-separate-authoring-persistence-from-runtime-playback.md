...
- Running and paused operate on runtime state. System mutations, runtime entities, hierarchy edits, and project-resource previews made there are not candidates for scene persistence. Eligible completed editor field and transform gestures on authored components are staged separately from runtime state. Copying remains editor-local. Duplicate and Paste stage authored subtree creation, while Delete and Cut stage authored subtree deletion; runtime members affected by the same live operation remain disposable. Declared write access alone does not make every instance of a component system-owned: the world records the specific entity/component pairs actually changed by project systems. Those pairs become ineligible for the remainder of that playback world, and deleting an authored entity drops any staged component edit for that UUID before Stop restores the baseline.
...
- Stop discards playback mutations and runtime-spawned entities by restoring that in-memory authoring baseline. It then replays staged structural creation/deletion batches in operation order before eligible staged component snapshots. Each structural operation becomes one undoable authoring transaction; deletion history recaptures the restored baseline snapshots so Undo cannot preserve system-mutated playback values. It preserves unsaved authored entities, dirty candidates, undo history, and UUID selection while retaining loaded Luau and Odin code, registered systems, and render resources.
...
The editor has a predictable model: Save commits authored state to disk, while Stop restores the authoring state that Play captured plus explicitly staged, eligible editor edits. Simulation-owned mutations remain disposable. Stable UUID lookup makes duplicate entity names safe, restores selection even when runtime slots change, and keeps persistence independent of list order in the live world. Semantic comparison prevents unchanged floating-point values and reverted edits from churning source text. The project transaction prevents a failed multi-file Save from exposing a partially updated authored state.
