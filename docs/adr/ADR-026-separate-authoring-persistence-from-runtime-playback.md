...
- Running and paused operate on runtime state. System mutations, runtime entities, playback deletes, hierarchy edits, and project-resource previews made there are not candidates for scene persistence. Eligible completed editor field and transform gestures on authored components are staged separately from runtime state. Duplicating an authored subtree is the bounded structural exception: its new scene-origin snapshots are staged as one batch, remain eligible for component editing, and are kept on Stop. Declared write access alone does not make every instance of a component system-owned: the world records the specific entity/component pairs actually changed by project systems. Those pairs become ineligible for the remainder of that playback world, and a later system claim discards an already staged conflicting edit before Stop restores the baseline.
...
- Stop discards playback mutations and runtime-spawned entities by restoring that in-memory authoring baseline. It then reapplies staged authored-duplication batches before eligible staged component snapshots, committing the result as one undoable authoring transaction. It preserves unsaved authored entities, dirty candidates, undo history, and UUID selection while retaining loaded Luau and Odin code, registered systems, and render resources.
