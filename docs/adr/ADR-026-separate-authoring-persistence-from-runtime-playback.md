...
- Running and paused operate on runtime state. System mutations, structural changes, runtime entities, and project-resource previews made there are not candidates for scene persistence. Eligible completed editor field and transform gestures on authored components are staged separately from runtime state. Declared write access alone does not make every instance of a component system-owned: the world records the specific entity/component pairs actually changed by project systems. Those pairs become ineligible for the remainder of that playback world, and a later system claim discards an already staged conflicting edit before Stop restores the baseline.
