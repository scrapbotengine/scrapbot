...
- `machina.input.pointer` stores pointer position, primary button edge/held state, and wheel delta.
- `machina.input.keyboard` stores modifier state plus editor-toggle edge state.
...
- The first keyboard representation is intentionally small. Full key state, text input, focus, capture, action mapping, and controller devices remain future design work.
