...
Store commands in a fixed-capacity editor-owned history containing the generation-aware target entity, component field, optional Vec3 axis, custom-component storage coordinates, and before/after numeric values. A new edit truncates the redo tail. `Ctrl/Cmd+Z` applies the previous value and `Ctrl/Cmd+Shift+Z` reapplies the next value while the editor is open; shortcuts do not consume project input when editor chrome is closed or a project-owned input has focus.
...
Commands currently cover the inspector's numeric transform, camera, light, and custom Vec3 fields only. Entity destruction or component removal can make an older command inapplicable; applying that command then leaves the history cursor unchanged. The fixed history drops its oldest entry when full, and edits are lost when the process exits.
