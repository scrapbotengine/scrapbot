...
The dynamic library is a development-time loading mechanism. The registration entrypoint and `machina_native` API are intentionally source-level concepts that a future `machina build` can call through a statically linked game module on platforms where dynamic loading is impossible or forbidden.
