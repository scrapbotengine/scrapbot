...
The implementation retains a small resolved cache in the resource registry and a retained singleton index/revision in the World. Volumetric media is now a separate authored `scrapbot.volumetric_fog` component consumed by postprocessing. Tone mapping, clouds, and other post effects should follow the same separation rather than turning World Environment into an unbounded render-settings bag.
