...
24. Port `wgpu-native` bindings and offscreen render verification. The first Odin slices should establish ABI-compatible scalar types, string/chained structs, renderer texture formats, usage flags, texture/buffer descriptors, offscreen copy/readback procedure types, typed proc-table resolution, and dynamic library loading before wiring real WebGPU device/render execution.
