...
25. Port retained UI, editor routing, and headful window integration. The first Odin headful slices establish SDL3 video initialization, hidden high-DPI window creation, native surface descriptor extraction, one-frame hidden WebGPU surface presentation smoke coverage, and bounded hidden scene presentation before visible presentation and the unbounded window loop.
