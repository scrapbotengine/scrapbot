...
24. Port `wgpu-native` bindings and offscreen render verification. The first Odin slices should establish ABI-compatible scalar types, string/chained structs, renderer texture formats, usage flags, texture/buffer descriptors, offscreen copy/readback procedure types, and typed proc-table resolution before adding foreign procedure declarations and platform library linking.
