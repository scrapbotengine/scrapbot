...
- **Stable-frame behavior:** Reuses postprocess pipelines, bind groups, resolution-scaled AO/fog targets, full-resolution surface/reflection targets, color/depth history, and the GPU-resident exposure scalar at stable size. Viewport resize, sampled-depth replacement, or an authored AO/fog resolution-scale change explicitly recreates size-dependent targets. World replacement, a detected camera cut, or a TAA-mode change rejects history. Disabled automatic exposure, AO, SSR, fog, and bloom skip their compute passes; disabled TAA skips projection jitter and history copies.
