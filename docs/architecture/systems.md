...
- **Outputs:** A geometric-error camera frontier with a narrow adjacent-level overlap, visible instance lists or a compact camera instance/cluster stream, indirect draw arguments, explicit hierarchy/frustum/occlusion rejection counters, LOD counters, asynchronous page feedback, active admission state, blended-cluster counts, and next-pass state. Completed requests admit page groups with a fresh current-frame grace window before sampled visible-use touches govern eviction.
- **Stable-frame behavior:** Does not rebuild membership, hierarchy products, shared geometry arenas, or submission policy; it encodes bounded GPU work over retained draw capacity. The projected-error overlap and active admission handoffs submit adjacent complete opaque levels while world and depth passes let ordinary depth testing retain the nearest surface. TAA consumes an internal HDR reactive marker for bounded transition depth tolerance. A compact transition queue advances only live admission handoffs, and completed start tokens do not keep obsolete parents selected. Native multi-draw adapters use the selected hierarchy for camera and cascade tests and share compatible arena-global command ranges. Other indirect-first-instance adapters append selected camera records into compatible material spans and increment one retained command per span. Their shadows use canonical indexed geometry when resident or a coarse indexed proxy built from pinned root pages; compact and classic culling own disjoint shadow commands. Diagnostics can transiently force the detailed native/emulated branch. Hi-Z samples a coarse mip covering the complete projected sphere footprint and skips unsafe camera-crossing or large near-field projections. CPU-reference mode is an explicit whole-primitive diagnostic substitute.
...
- **Inputs:** HDR world target, scene depth, octahedral view normal/roughness/metallic surface target, separately retained indirect diffuse light, optional singleton fog/vignette/lens-flare/lens-dirt components, primary directional light and shadow cascades, GPU-built clustered point-light buffers, active-camera fixed/automatic exposure and TAA/fast-AA/AO/SSR/bloom policy, current/previous camera projection state, retained temporal color/depth history, automatic-exposure/AO/reflection/fog/bloom resources, and presentation format.
- **Outputs:** Composited scene color in the presentation target.
- **Stable-frame behavior:** Reuses postprocess pipelines, bind groups, full-resolution surface/reflection targets, a half-resolution fog target, color/depth history, and the GPU-resident exposure scalar at stable size. Resize, sampled-depth replacement, world replacement, a detected camera cut, or a TAA-mode change rejects history. Disabled automatic exposure, AO, SSR, fog, and bloom skip their compute passes; disabled TAA skips projection jitter and history copies.
- **Boundary:** WGPU world shading resolves the primary directional light's cascaded shadows through a nine-comparison, wider tent kernel and blends adjacent cascades over stable transition bands. Postprocess encoding first integrates optional adaptive 4–16-step height/distance fog at half resolution with temporally rotated low-discrepancy sub-step offsets, 2×2 UV-space filtered primary-directional shadows, those same cascade transitions, and the complete relevant point-light list from each sampled GPU-built cluster. Temporal resolution depth-aware upsamples that retained fog signal before history integration. Postprocessing also performs optional half-resolution visibility-bitmask ambient occlusion, optional bounded view-space SSR, and temporal or current-frame resolution. Optional automatic exposure reduces 256 viewport-stratified log-luminance samples into one persistent GPU scalar without readback. The final composite combines bloom with bounded ghost/halo flare samples, modulates optical energy with deterministic procedural dirt, tone maps once, then applies vignette and fixed dithering. Adaptive post quality reduces ghost count. Non-lit debug views bypass presentation effects, while UI remains crisp and excluded.
