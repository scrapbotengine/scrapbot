...
| `scrapbot.gizmo` | Reconciles camera/light visualization, transform and terrain-cell tool interaction, and the editor-world overlay stream. | An editor UI state exists; most work is conditional on editor visibility, selection, and the active world tool. | `render/render.odin`, `render/gizmo.odin`, `render/terrain_tool.odin`, `render/camera_visualizer.odin`, `render/scene_icons.odin` |
...
| `scrapbot.render.cull` | Encodes GPU visibility, resident virtual-geometry frontier overlap and admission handoffs, demand/predictive group feedback, frustum/Hi-Z rejection, LOD selection, and indirect visibility compaction. | WGPU frames; completed feedback may admit or evict complete vertex/index page groups within frame limits. CPU-reference mode substitutes CPU culling and whole-primitive LOD selection. | `render/wgpu_gpu_driven.odin`, `render/wgpu_visibility.odin`, `render/wgpu_hiz.odin` |
| `scrapbot.render.shadow` | Encodes retained, staggered directional-shadow cascade refreshes. | WGPU frames with applicable shadow state; near and invalidated layers refresh immediately, while stable far layers follow bounded cadences. | `wgpu_encode_shadow_pass` |
...
- **Inputs:** Editor selection, resolved world transforms and selected light or authorable voxel-surface payload, active editor camera and viewport, window pointer state plus world-tool capture/wrap displacement, gizmo mode/orientation/pivot, transient snap increments, retained selected-subtree bounds, and compact camera/directional-light/point-light membership.
- **Outputs:** Immediate free modal translation/view-axis rotation/uniform scaling, later axis-qualified Transform edits resolved against the same captured gesture, increment-snapped origin- or bounds-center-based Transform edits, discrete terrain-cell edits with projected cell previews, camera body/frustum geometry, decluttered constant-size camera/light icons, selected point-range rings or directional arrow, canonical light-field edits, and the gizmo/overlay paint and interaction state.
- **Stable-frame behavior:** Reuses retained overlay GPU output and a selected-subtree bounds center until exact projected output or interaction state changes. CPU projection visits only compact active camera/light membership plus bounded selected-tool geometry. Terrain targeting performs one exact query against the selected geometry and projects one 12-edge cell preview. Icon decluttering is capped at 256 entries and the selected light adds at most 144 range segments or one direction segment.
- **Boundary:** Main-thread CPU editor tooling; visualization is an overlay consumer, not an authored renderable entity. Initial hits respect viewport/chrome ownership, while active transform and light gestures retain the unfiltered window pointer until commit or cancellation. An active terrain-cell tool suppresses the transform gizmo and consumes its successful click.
- **Source/tests:** `render/gizmo.odin`, `render/terrain_tool.odin`, `render/light_gizmo.odin`, `render/camera_visualizer.odin`, `render/scene_icons.odin`, `ui/editor_ecs.odin`; `render/gizmo_test.odin`, `resources/resources_test.odin`, `render/light_gizmo_test.odin`, `render/camera_visualizer_test.odin`, `render/scene_icons_test.odin`, `tests/fixtures/ui/voxel-terrain-cell-authoring.json`.
...
- **Invalidation:** Light activation, world or batch-topology replacement, Transform mutation, camera discontinuity, and shadow-resolution changes force all four layers current.
...
- **Inputs:** HDR world target, scene depth, octahedral view normal/roughness/metallic surface target, separately retained indirect diffuse light, depth-independent editor selection/placement mask, optional singleton fog/vignette/lens-flare/lens-dirt components, primary directional light and shadow cascades, GPU-built clustered point-light buffers, active-camera fixed/automatic exposure and TAA/fast-AA/AO/SSR/bloom policy, current/previous camera projection state, retained temporal color/depth history, automatic-exposure/AO/reflection/fog/bloom resources, and presentation format.
- **Outputs:** Viewport-local postprocessed scene color mapped into the physical game viewport of the presentation target.
- **Stable-frame behavior:** Reuses postprocess pipelines, bind groups, resolution-scaled AO/fog targets, full-resolution surface/reflection targets, color/depth history, and the GPU-resident exposure scalar at stable size. Viewport resize, sampled-depth replacement, or an authored AO/fog resolution-scale change explicitly recreates size-dependent targets. World replacement, a detected camera cut, a TAA-mode change, or an actual retained-instance upload rejects history; the instance rule prevents camera-only reprojection from retaining moved geometry until object motion vectors exist. Disabled automatic exposure, AO, SSR, fog, and bloom skip their compute passes; disabled TAA skips projection jitter and history copies.
- **Boundary:** WGPU world shading resolves the primary directional light's cascaded shadows through a nine-comparison, wider tent kernel and blends adjacent cascades over stable transition bands. World, depth, and post targets are sized from the physical game viewport rather than editor chrome; their internal viewport starts at the local target origin. Postprocess encoding first integrates optional adaptive 4–16-step height/distance fog at its authored target scale with temporally rotated low-discrepancy sub-step offsets, 2×2 UV-space filtered primary-directional shadows, those same cascade transitions, and the complete relevant point-light list from each sampled GPU-built cluster. Temporal resolution reconstructs that retained fog signal from actual source/target dimensions before history integration. Postprocessing also performs optional resolution-scaled visibility-bitmask ambient occlusion with depth/normal-aware reconstruction, optional bounded view-space SSR, and temporal or current-frame resolution. Optional automatic exposure reduces 256 viewport-stratified log-luminance samples into one persistent GPU scalar without readback. The final composite maps that local target into the physical game viewport, combines bloom with bounded ghost and distance-preserving halo flare samples, modulates optical energy with deterministic procedural dirt, tone maps once, applies vignette, composites the editor-only through-occluder selection outline or placement ghost, and adds fixed dithering. Adaptive post quality reduces ghost count. Non-lit debug views bypass presentation effects, while UI remains crisp and excluded.
