...
| `scrapbot.render.cull` | Encodes GPU visibility, resident virtual-geometry frontier selection and page requests, frustum/Hi-Z rejection, LOD selection, and indirect visibility compaction. | WGPU frames; completed page feedback may upload or evict index pages. CPU-reference mode substitutes CPU culling and whole-primitive LOD selection. | `render/wgpu_gpu_driven.odin`, `render/wgpu_visibility.odin`, `render/wgpu_hiz.odin` |
