...
Model previews add one editor-bound public button while stopped, and Model resource rows remain generic public drag sources. Editor orchestration records the resource, optional parent, and optional pointer as a bounded placement request. After UI reconciliation, the renderer builds a camera ray from the current Game viewport and editor fly camera, asks the backend-neutral scene query for the nearest exact triangle, and falls back to the world ground plane or a five-unit focus point. Optional world-grid quantization is applied before the UI authoring transaction creates the scene-origin Transform/Model root. Selection, dirty candidates, parenting, and structural history remain in editor authoring; model-derived runtime children remain the ordinary change-driven reconciliation consumer.
