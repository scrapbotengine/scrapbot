...
Input singletons are committed once before the schedule runs. Luau, native systems, retained UI, editor actions, and scene-camera navigation read the same immutable held/pressed/released snapshot without allocating synthetic entities or scanning entity storage. SDL contributes focused text composition and repeated text-navigation commands beside the snapshot and remains responsible for OS relative-mode, cursor, confinement, and warp services; it does not decode semantic editor commands.
