...
The backend-neutral scene query consumes a ray, retained render instances, each Geometry's bounds, and its position-only query proxy. It rejects transformed instance AABBs first. Streamed virtual Geometry reuses retained hierarchy-group and leaf-cluster spheres as an intra-mesh broad phase before exact triangle tests. It returns the nearest entity, world position, geometric normal, and distance. Viewport selection projects its pointer into the same query and reads only the entity. Placement reads the complete hit, keeping spatial policy out of reusable UI interaction while avoiding a second triangle traversal implementation.
