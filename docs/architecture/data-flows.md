...
Pressing G, R, or S starts a modal transform immediately from the current pointer and captured world Transform. Its initial handle is free camera-plane translation, frozen view-axis rotation, or uniform scaling. A later X/Y/Z edge replaces that active handle and re-evaluates the complete pointer delta or angle from the same captured start against the requested frozen world/local axis or complementary plane; it does not create or commit a second gesture. While the modal transform is active, the platform boundary confines and edge-wraps the visible pointer. Render orchestration applies each warp displacement to the retained start and previous-pointer anchors after the current motion is evaluated, keeping translation, rotation, and scale continuous while ordinary UI retains physical in-window coordinates.
