...
Model previews add one editor-bound public button while stopped, and Model resource rows remain generic public drag sources. During an active valid drag, editor orchestration reads the retained generic source, target, and current pointer. The renderer builds a camera ray from the current Game viewport and editor fly camera, asks the backend-neutral scene query for the nearest exact triangle, and falls back to the world ground plane or a five-unit focus point. It quantizes the contact, centers transformed Model bounds around it in the surface plane, offsets the root along the hit normal by their minimum projection, and projects the contact/root pair into the editor-world overlay. The WGPU editor-feedback pass draws the Model resource's complete fallback primitives directly at that transform, producing a translucent ghost without temporary ECS entities or Model reconciliation. Release repeats the same query before the UI authoring transaction creates the scene-origin Transform/Model root. Active drags may query and redraw each input frame; stable frames retain no placement work. Selection, dirty candidates, parenting, and structural history remain in editor authoring; model-derived runtime children remain the ordinary change-driven reconciliation consumer.

The backend-neutral scene query consumes a ray, retained render instances, and each Geometry's position-only query proxy. It returns the nearest entity, world position, geometric normal, and distance. Viewport selection projects its pointer into the same query and reads only the entity. Placement reads the complete hit, keeping spatial policy out of reusable UI interaction while avoiding a second triangle traversal implementation.

The frame orchestration resolves the selected entity's UUID after picking. WGPU compares that transient identity with retained render instances and their `model_owner` UUIDs. A selection change patches the previous/new instance flags and rebuilds a dense selected-slot set once; an ordinary dirty-instance update changes only that slot's flag and membership. After world shading, an editor-only pass draws those slots' complete fallback Geometry into a two-channel mask without attaching scene depth. The final composite dilates the first channel into an occlusion-independent amber silhouette and uses the second for the active placement ghost. Running and Paused mask the selection silhouette into screen-space dashes whose renderer-owned phase advances with frame delta independently of ECS simulation time; Stopped keeps it solid. Hiding the editor retains selection identity but requests no feedback draws, causing one clear of an active mask before stable hidden frames skip the pass. Stable visible selection performs no instance scan or membership rebuild; the mask is redrawn with the frame because camera and geometry transforms remain frame-valued.
