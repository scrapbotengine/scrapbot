...
| `src/scrapbot/asset_import/` | Incremental source-asset importers, common product envelopes, atomic products, SVG-to-MTSDF icon compilation, texture mips, static glTF decoding/LOD simplification, compact streaming Model catalogs, and HDR-to-IBL preprocessing. | Source/dependency/settings fingerprints and versioned `.scrapbot/imported/` products; typed chunk ranges are validated before importer decoding, page payload ranges are skipped during cache-hit catalog loading, and source conversion is never ordinary-frame work. |
