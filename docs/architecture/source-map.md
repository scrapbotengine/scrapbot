...
| `src/scrapbot/asset_import/` | Incremental source-asset importers, common product envelopes and sequential writers, atomic products, SVG-to-MTSDF icon compilation, texture mips, static glTF decoding/LOD simplification, split streaming Model products, and HDR-to-IBL preprocessing. | Source/dependency/settings fingerprints and versioned `.scrapbot/imported/` products; typed chunk ranges are validated before importer decoding, Model catalogs/images/coarse pages/detail pages are independently addressable, and source conversion is never ordinary-frame work. |
| `src/scrapbot/geometry/` | Backend-independent crack-aware hierarchy construction, validation, cloning, and deterministic self-contained vertex/index page construction. | Imported products and runtime Geometry use one representation; no renderer ownership or ordinary-frame construction. |
...
| `src/scrapbot/render/` | Backend interface, null backend, surface/offscreen WGPU rendering, shared geometry arenas, bounded virtual-geometry residency, projected-error frontier overlap and streamed-admission transitions, GPU-driven object/cluster visibility, native multi-draw and parallel portable compacted submission, proxy-backed picking, gizmos, embedded UI viewports, postprocess, and bounded profile collection. | Backend-neutral inputs; WGPU consumes temporary canonical views only at cache invalidation, then owns geometry-versioned arena/page ranges, pinned and refinement-protected fallback residency, a compact admission queue, indexed current/future-camera demand and use feedback, one priority-ordered group-atomic eviction plan per completed feedback batch, targeted cluster-state patches, device-limit-aware storage-table capacity, baseline-limit candidate/camera/shadow compact passes, capability-gated retained commands, coordinated budget-driven world/shadow/post quality, pooled adaptive viewport targets, isolated resource-preview scenes, queue-drained capture readback, and tagged asynchronous timing/visibility readbacks. |
