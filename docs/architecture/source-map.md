...
| `src/scrapbot/render/` | Backend interface, null backend, surface/offscreen WGPU rendering, shared geometry arenas, bounded virtual-geometry residency, GPU-driven object/cluster visibility, native multi-draw and parallel portable compacted submission, proxy-backed picking, gizmos, embedded UI viewports, postprocess, and bounded profile collection. | Backend-neutral inputs; WGPU consumes temporary canonical views only at cache invalidation, then owns geometry-versioned arena/page ranges, pinned and refinement-protected fallback residency, indexed current/future-camera demand and use feedback, one priority-ordered group-atomic eviction plan per completed feedback batch, targeted cluster-state patches, device-limit-aware storage-table capacity, baseline-limit candidate/camera/shadow compact passes, capability-gated retained commands, coordinated budget-driven world/shadow/post quality, pooled adaptive viewport targets, isolated resource-preview scenes, queue-drained capture readback, and tagged asynchronous timing/visibility readbacks. |
