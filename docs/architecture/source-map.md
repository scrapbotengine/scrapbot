...
| `src/scrapbot/render/` | Backend interface, null backend, surface/offscreen WGPU rendering, shared geometry arenas, GPU-driven object/cluster visibility, native multi-draw and portable compacted submission, picking, gizmos, embedded UI viewports, postprocess, and bounded profile collection. | Backend-neutral inputs; WGPU owns geometry-versioned arena ranges, capability-gated retained commands and compact record streams, pooled adaptive viewport targets, isolated resource-preview scenes, optional capture readback, and tagged asynchronous timing/visibility readbacks. |
