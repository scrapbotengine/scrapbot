...
| `src/scrapbot/asset_import/` | Incremental source-asset importers, atomic products, SVG-to-MTSDF icon compilation, texture mips, static glTF decoding/LOD simplification, compact streaming Model catalogs, and HDR-to-IBL preprocessing. | Source/dependency/settings fingerprints and versioned `.scrapbot/imported/` products; page payload ranges are skipped during cache-hit catalog loading; never ordinary-frame work. |
