...
| `src/scrapbot/render/` | Backend interface, null backend, surface/offscreen WGPU rendering, GPU-driven object/meshlet visibility and indirect submission, picking, gizmos, embedded UI viewports, postprocess, and bounded profile collection. | Backend-neutral inputs; WGPU owns geometry-versioned canonical/meshlet index caches, capability-gated draw state, pooled adaptive viewport targets, isolated resource-preview scenes, optional capture readback, and tagged asynchronous timing/visibility readbacks. |
