...
| `src/scrapbot/render/` | Backend interface, null backend, surface/offscreen WGPU rendering, shared geometry arenas, bounded virtual-geometry residency, GPU-driven object/cluster visibility, native multi-draw and portable compacted submission, proxy-backed picking, gizmos, embedded UI viewports, postprocess, and bounded profile collection. | Backend-neutral inputs; WGPU consumes temporary canonical views only at cache invalidation, then owns geometry-versioned arena/page ranges, pinned fallback residency, current/future-camera demand and use feedback, demand-first group-atomic admission/eviction, device-limit-aware storage-table capacity, capability-gated retained commands and compact record streams, pooled adaptive viewport targets, isolated resource-preview scenes, optional capture readback, and tagged asynchronous timing/visibility readbacks. |
