...
| `src/scrapbot/resources/` | Runtime geometry/texture/environment/icon-set/model/material/font/UI-theme registries and generational handles. | Shared descriptions outside ECS; UI/backend caches consume render-resource versions, while theme versions serve explicit composition and editor inspection only. |
...
| `src/scrapbot/ui/` | Retained ECS UI, intrinsic/flex layout, interaction, editor ECS composition, runtime component-payload inspection/bindings, diagnostics, fonts. | Layout and paint share active-font line breaking; wrapped flex work is bounded and revision-driven. Editor recipes consume the shared public contract before storage; editor meaning stays in bindings/orchestration. |
