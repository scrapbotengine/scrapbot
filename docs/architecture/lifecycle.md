...
| Scene entity creation | Editor `+`, duplicate, Model-preview placement, structural Undo/Redo | Create scene-origin entity with a new/stored UUID and scene order; Model placement adds one camera-relative Transform/Model root; add authored components through authoring snapshots | Existing resource UUIDs resolve through the live registry and remain persistent authority | No runtime/code reload | Exact UI/render membership and editor browsers become dirty; placed roots become selected | Failed operation leaves history/source baseline unchanged |
