...
| --- | --- | --- | --- | --- | --- | --- |
| Project load | `scrapbot run`, check/build paths needing a full project | Parse/validate resources before the scene, resolve built-in-name or UI-theme-UUID directives before explicit component overrides, build scene-origin entities with stable UUIDs, reconcile imported Model roots into derived runtime children, bootstrap structural/render/UI dirtiness | Ensure Texture/Model/Environment/Icon Set products, including invalidated offline model LOD generation; read the Model catalog and image ranges while retaining coarse/detail Geometry pages as file ranges; register and promptly release decoded materials, primitives, meshes, and nodes; register remaining built-ins, fonts, textures, environments, icon sets, authored materials, LOD geometry, and UI themes; resolve ECS handles | Build native extensions when needed, register engine/native/project components, initialize shared deferred-command storage even when Luau is absent, execute Luau when present, validate world | An automatic editor or opt-in live debug service publishes import/initialization phases before UI/render consumers receive resolved public components and bootstrap from explicit initial dirtiness | Abort before renderer loop; atomic importer writes retain prior valid products; live debug startup failure aborts and unwinds its service state |
| Scene entity creation | Stopped-mode editor `+`, duplicate, Model-preview placement, structural Undo/Redo | Create scene-origin entity with a new/stored UUID and scene order; Model placement adds one camera-relative Transform/Model root; add authored components through authoring snapshots | Existing resource UUIDs resolve through the live registry and remain persistent authority | No runtime/code reload | Exact UI/render membership and editor browsers become dirty; created roots become selected | Failed operation leaves history/source baseline unchanged |
...
| Pause | Pause control/shortcut or opening the editor during playback | Preserve current simulated ECS values; no simulation delta advances | Preserve current runtime resource values | Runtime stays loaded; scheduled project systems do not advance | Editor/UI/render frames may continue for inspection; play-mode warning remains because edits are disposable | No persistence boundary |
...
| Project/world hot reload | Project, scene, asset, resource, extension binary/source stamp changes | Build and validate a next world from disk, explicitly recompose scene theme directives, reconcile Model-derived children, then replace active world | Ensure imports; re-register fonts/textures/environments/icon sets/models/materials/LOD geometry/UI themes in stable slots; increment versions/generations/topology as required | Rebuild native extensions and script runtime, then atomically replace runtime/extension/schedule ownership | New world bootstraps structural/render/UI state; backend caches follow render-resource revisions; no live theme traversal occurs | Failed imports preserve product files; failed replacement destroys candidate world/runtime and retains/restores the last-good runtime |
