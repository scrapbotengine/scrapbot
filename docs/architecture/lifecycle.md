...
| --- | --- | --- | --- | --- | --- | --- |
| Project load | `scrapbot run`, check/build paths needing a full project | Parse/validate scene, resolve UI theme directives before explicit component overrides, build scene-origin entities with stable UUIDs, reconcile imported Model roots into derived runtime children, bootstrap structural/render/UI dirtiness | Ensure Texture/Model/Environment/Icon Set products; register built-ins, fonts, textures, environments, icon sets, models, generated model subresources, authored materials and LOD geometry; resolve ECS handles | Build native extensions when needed, register engine/native/project components, initialize shared deferred-command storage even when Luau is absent, execute Luau when present, validate world | UI/render consumers receive only resolved public components and bootstrap from explicit initial dirtiness | Abort before renderer loop; atomic importer writes retain prior valid products |
