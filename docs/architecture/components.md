...
| `scrapbot.camera` | Spatial/render | Authored | Yes | Selects camera projection, world render-grid scale, fixed or GPU-adaptive exposure, and per-view TAA/fast-AA/AO/SSR/bloom policy; project camera is distinct from the editor fly camera. |
...
- **Contract:** Perspective projection, a bounded `0.5`–`1` world render-grid scale, positive fixed exposure or opt-in GPU-adaptive exposure, and per-view temporal-AA, fast-AA, ambient-occlusion, screen-space-reflection, and bloom policy attached to an entity whose Transform supplies the project-camera pose. AO and SSR expose bounded `0.25`–`1` sample-quality tiers. Automatic exposure clamps to authored positive bounds, adapts at an authored positive inverse-seconds rate, and treats manual exposure as compensation. TAA takes precedence over fast AA. Zero-valued legacy/programmatic components normalize to native render scale `1`, fixed exposure `1`, automatic bounds `0.125`–`8`, speed `2`, and balanced AO/SSR quality `0.5`; authored TOML defaults automatic exposure off, TAA/AO/bloom on, and fast AA/SSR off.
- **Storage/lifecycle:** Dedicated typed ECS storage; authored.
- **Producers:** Scene loading, editor/component authoring, validated Luau query writeback, and native membership commands.
- **Consumers:** Active-camera selection, scaled world/depth/post target layout, render-view construction, postprocess dispatch/jitter/history/exposure policy, global environment uniform, visible-sky ray construction, editor camera mesh/frustum visualization, and scene picking.
- **Invalidation:** Membership is structural; projection, render scale, exposure, render-feature, quality, or Transform changes update compact camera input. Render-scale changes lazily replace only the owned scaled depth and size-dependent post targets and reject temporal history; native scale reuses the output depth target. Fixed exposure changes rewrite only the environment uniform. Automatic exposure uses a persistent GPU scalar and one bounded metering dispatch per enabled frame; disabling it restores scalar `1` once and performs no metering work. AO and SSR quality change only the next uniform and loop bound; neither rebuilds retained post targets. TAA mode changes reject temporal history; disabled AO/SSR/bloom skip their compute work. The editor fly camera contributes pose/lens while inheriting the active project camera's complete render policy.
