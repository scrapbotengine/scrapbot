...
| Live debug snapshot and capture job | Root-owned `live_debug.Service`; published by renderer frame boundaries | Immutable derived diagnostics plus one bounded pending command | Windowed editor lifetime or explicit `--live-debug` opt-in. The engine thread replaces one owned snapshot and consumes at most one 1–16-frame telemetry capture. A loopback HTTP worker only authenticates, encodes, and updates mutex-protected request state; it never reads ECS, registry, renderer, or GPU owners directly. Stable frames perform one bounded snapshot copy only while enabled. Shutdown removes discovery and releases captured strings. See ADR-053. |
