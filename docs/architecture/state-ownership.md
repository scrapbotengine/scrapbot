...
The extracted camera's debug-view value is authoritative for a rendered view. While the editor is visible, an optional transient Game-view choice replaces that value only on the retained render-list camera copy. Non-lit output updates the compact render uniform, skips temporal and presentation effects, and reuses existing targets. Meshlet identity storage follows meshlet topology/capacity invalidation rather than frames.
