...
| Authoring history, dirty UUID candidates, and staged play edits | Editor UI state | In-memory authoring authority until Save/Revert plus transient per-play journal | One transaction per completed gesture or structural operation. Structural batches own ordered entity snapshots, scene order, and before/after selection so subtree Duplicate/Delete apply atomically. During playback, eligible completed editor gestures replace one latest component snapshot keyed by entity UUID and component identity. The first actual project-system claim refreshes only that entity's Inspector eligibility; Stop discards newly conflicted entries before restoring the baseline and applies the remainder as one component-batch history transaction. Stable frames do no journal work. |
