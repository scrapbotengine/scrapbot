...
| Authoring history, dirty UUID candidates, and staged play edits | Editor UI state | In-memory authoring authority until Save/Revert plus transient per-play journal | One transaction per completed gesture or structural operation. Structural batches own ordered entity snapshots, scene order, and before/after selection so subtree Duplicate/Delete apply atomically. During playback, an authored duplicate retains its scene origin and stages one structural batch, while eligible completed gestures replace one latest component snapshot keyed by entity UUID and component identity. Runtime-origin duplicates remain disposable. The first actual project-system claim refreshes only that entity's Inspector eligibility; Stop restores staged structural batches before their component snapshots, discards newly conflicted component entries, and pushes the surviving batches into stopped-mode history. Stable frames do no journal work. |
