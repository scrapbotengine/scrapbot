...
| Placement preview and rendered selection cue | Editor UI state plus WGPU retained instance records, dense selected slots, two-channel feedback mask, and selection-outline phase | Transient editor-derived state | An active valid Model drag resolves its frame-valued pointer through the backend-neutral scene query, rebuilds the editor overlay when contact/root changes, and draws complete Model fallback primitives into the ghost channel. Release clears the preview. Selection UUID changes scan current retained instances once and patch direct or `model_owner` matches plus dense membership; later dirty slots update locally. The depth-independent mask redraws selected geometry each frame but stable selection performs no membership scan or instance upload. During Running and Paused, renderer frame delta advances the dashed selection phase independently of simulation; Stopped resets the phase and retains the solid outline. |
