...
| UI theme palettes, metrics, and recipes | UI composition | Ephemeral composition input, not runtime authority | Applied explicitly to canonical component defaults before typed ECS attachment or update. The resolved `ui_*` values are authoritative for layout and paint; no theme identity, ancestry cascade, stable-frame traversal, or renderer-side style store remains. |
