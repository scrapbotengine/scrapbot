...
| Editor shell transport and workspace visibility | Editor UI state | Transient per-run tool state | Opening the shell pauses active playback and closing resumes it. Command-modified sidebar toggles flip retained left/right visibility booleans; a visual-state comparison mutates only the affected public dock-space layout, so stable frames perform no repeated workspace mutation. Playback-state changes update the retained viewport badge, frame, and status text. Running alone advances the public viewport border dash phase and causes a targeted paint update; Paused holds a solid frame and Stopped clears it. |
| Placement preview and rendered selection cue | Editor UI state plus WGPU retained instance records, dense selected slots, and two-channel feedback mask | Transient editor-derived state | An active valid Model drag resolves its frame-valued pointer through the backend-neutral scene query, rebuilds the editor overlay when contact/root changes, and draws complete Model fallback primitives into the ghost channel. Release clears the preview. Selection UUID changes scan current retained instances once and patch direct or `model_owner` matches plus dense membership; later dirty slots update locally. The depth-independent mask redraws selected geometry each frame but stable selection performs no membership scan or instance upload. |
