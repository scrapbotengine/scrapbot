...
| Editor scene-camera pose, orbit point, and focus request | Transient editor-camera ECS entity plus editor UI state | Per-run tool state with one-shot derived framing input | Captured fly/orbit movement and viewport-owned wheel input mutate only the editor camera. The internal camera component retains one point of interest and distance: fly translation carries the point, free-look retargets it, wheel input changes distance, and middle-mouse input orbits it. Accepted `F` input sets one request bit; the next camera phase clears it, visits retained instances for the selected entity/subtree once, derives combined world bounds or a Transform fallback, updates the camera pose, and replaces the orbit point. Stable frames perform no selection-bounds traversal. |
| Transform-gizmo pivot and selected bounds center | Transient `EditorTransformGizmo` plus editor UI state | Derived editor manipulation policy and cache | Origin uses the selected resolved Transform position. Center caches combined bounds from current retained render instances and invalidates on selection/World changes, render hierarchy/topology, Geometry topology, or dirty selected/model-owned/descendant render entities. A gesture freezes the pivot; translation updates the cached center by its solved delta, while rotation and scale retain it. Stable frames perform no render-instance traversal. |
| Placement preview and rendered selection cue | Editor UI state plus WGPU retained instance records, dense selected slots, two-channel feedback mask, and engine-time sampling | Transient editor-derived state | An active valid Model drag resolves its frame-valued pointer through the backend-neutral scene query, rebuilds the editor overlay when contact/root changes, and reuses each primitive's resolved conventional data or virtual pinned proxy in the ghost channel. Release clears the preview. Selection UUID changes scan current retained instances once and patch direct or `model_owner` matches plus dense membership; later dirty slots update locally. The depth-independent mask redraws selected geometry from the same submission cache each frame, without changing Geometry cache policy, but stable selection performs no membership scan or instance upload. During Running and Paused, dashed selection samples run-global engine elapsed time independently of simulation; Stopped retains the solid outline. Hiding the editor retains selection but clears an active mask once, then skips stable hidden feedback passes. |
