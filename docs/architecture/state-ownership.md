...
| Geometry/material/environment/icon-set descriptions and handles | `resources.Registry` | Runtime shared-resource authority | Generational handles plus content/topology versions. Geometry content includes registration-built meshlet streams and bounds; they rebuild only with that exact Geometry version, never on stable frames. See [Resource render state](#resource-render-state). |
...
| GPU instances, whole-primitive and meshlet draw/visibility data, lights, shadows, postprocess targets, pipelines, and resource caches | WGPU backend | Derived backend state | Exact dirty queues, resource versions, camera/viewport revisions, target shape, capacity growth, world replacement, or backend lifetime. Meshlet index/metadata/template state follows Geometry versions and retained batch capacity; current-frame counts are reset from retained templates. Windowed runs add an SDL-backed surface; headless runs own only an offscreen color target and allocate readback storage on explicit capture. See [WGPU derived state](#wgpu-derived-state). |
| Active-camera render policy | Authored `scrapbot.camera`; consumed by WGPU | Authoritative ECS policy with derived dynamic-scale, target/execution/history, and exposure state | Manual scale is the dynamic ceiling; minimum scale and target GPU milliseconds bound the optional controller. Completed asynchronous timestamps are consumed individually and exactly once, filtered, and stepped with hysteresis after subtracting native UI time. Samples are tagged with a generation; scale/policy changes and a different stable project-camera UUID reset state and reject delayed evidence. Unsupported timing falls back to the ceiling. The editor fly camera contributes pose/lens but retains the project camera as policy owner. Effective scale selects a retained world/depth/post grid while final composition and UI use the native output grid. A scale step replaces only size-dependent targets and rejects history; stable scale reuses them. Automatic exposure owns one persistent GPU scalar and performs no CPU readback; disabling it restores scalar `1` once. TAA-mode changes reject history; disabled TAA omits jitter and history sampling, while disabled automatic exposure/AO/SSR/bloom omit their compute dispatches. AO quality updates one uniform and bounded shader loop, without target recreation. Retained postprocess targets stay allocated across feature toggles to avoid allocation churn. |
...
buffers, aligned camera/shadow visibility slices, a parallel debug-identity stream, and indirect templates. Geometry versions define
meshlet command topology. Batch membership capacity defines visibility allocation. Stable frames
...
The extracted camera's debug-view value is authoritative for a rendered view. While the editor is visible, an optional transient Game-view choice replaces that value only on the retained render-list camera copy. Non-lit output updates compact render/cull uniforms, skips temporal and presentation effects, and reuses existing targets. Meshlet identity storage follows meshlet topology/capacity invalidation rather than frames. Its visible-instance allocation reserves an aligned diagnostic tail. Visibility mode alone writes rejected bounds there during the ordinary frame-valued cull, copies the counter into an indirect line draw, and publishes the count asynchronously; other modes perform no diagnostic writes or overlay pass.
