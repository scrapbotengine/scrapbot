...
| Deferred structural commands | Per-runtime and per-native-worker `ecs.Command_Buffer` | Ordered pending lifecycle mutations | Compact headers reference separate typed payload arrays; spawn component side arrays contain only present payloads. Arrays grow geometrically, merge with index remapping in schedule order, clear without releasing capacity each frame, and release at runtime/cache destruction. |
