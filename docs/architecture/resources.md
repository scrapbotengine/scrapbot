...
- WGPU uploads an exact Geometry version into shared aligned vertex and index arena ranges with both draw and storage usage. Canonical and meshlet-ordered indices remain resident.
- Hierarchy-cluster indices use deterministic group-aligned pages. Complete resources that fit the remaining budget upload in one transfer; otherwise the coarsest frontier stays pinned. Bounded GPU feedback reports prioritized missing groups and visible resident-group touches.
- Completed feedback admits or evicts complete groups under per-frame byte/group limits and the project budget. Adapters with indirect-first-instance project hierarchy-group error into pixels and select the desired resident frontier.
- Native multi-draw submits retained per-cluster indexed commands. Other capable adapters compact instance/cluster records into bounded shared spans and vertex-pull from the arenas. Adapters without indirect-first-instance and capacity-limited layouts retain whole-primitive indexed-indirect submission.
