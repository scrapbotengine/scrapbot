...
- WGPU uploads an exact Geometry version into shared aligned vertex and index arena ranges with both draw and storage usage. Canonical and meshlet-ordered indices remain resident. Hierarchy-cluster indices use deterministic pages: the coarsest frontier stays pinned, GPU feedback requests finer pages, and least-recently-requested non-pinned pages are evicted under the project budget. Adapters with indirect-first-instance project hierarchy-group error into pixels and select the desired resident frontier. Native multi-draw submits retained per-cluster indexed commands; other capable adapters compact instance/cluster records into bounded shared spans and vertex-pull from the arenas. Adapters without indirect-first-instance and capacity-limited layouts retain whole-primitive indexed-indirect submission.
