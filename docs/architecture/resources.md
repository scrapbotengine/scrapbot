...
- WGPU uploads an exact Geometry version into shared aligned vertex and index arena ranges. Canonical, meshlet-ordered, and hierarchy-cluster indices share the index arena. Adapters with indirect-first-instance and native multi-draw project hierarchy-group error into pixels, select one fully resident cluster frontier, then consume retained bounds and cones through compute culling and fixed multi-draw submission. Unsupported or capacity-limited layouts retain whole-primitive indexed-indirect submission over the same arenas, avoiding CPU expansion of cluster command ranges.
