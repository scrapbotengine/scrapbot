...
- Model v16 has independent material-image, pinned-coarse-page, evictable-detail-page, and catalog chunks. Import streams coarse pages directly, uses a temporary detail-page spool, copies detail bytes with a fixed buffer, and emits the catalog one primitive record at a time. The reader validates all descriptor ranges against the expected chunk before registry publication.
