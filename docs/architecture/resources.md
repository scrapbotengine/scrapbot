...
- Every page has one self-contained canonical-vertex subset and page-local expanded index stream. Imported Geometry owns validated ranges in its immutable Model product; other producers own equivalent in-memory bytes. Clone, replacement, generated-LOD registration, retirement, and destruction move or release the source contract with the Geometry entry.
